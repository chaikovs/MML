function [g1,g3,CXZ, Np]=bunch_image(phasespace,nbin,w,diffusion)
% Return 2D desnisti image CXZ from phasespace binning
% W : window Xmax and Zmax

if nargin==4;
    phasespace([1 3],:)=phasespace([1 3],:)+randn(2,length(phasespace))*diffusion; % screen diffusion
end
...
%b=4;h=ones(b)/b^2;
%CXZ=filter2(h,CXZ);
%[CXZ] = image_ffthanning_cleaner(CXZ, 0.7, 0.3);
n1=length(g1);
n3=length(g3);
CXZ=CXZ(1:n3,1:n1);
