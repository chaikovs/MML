...
%
%   e-mail bug reports/comments to <a href="mailto:jiro.doke@mathworks.com">Jiro</a>
...
switch lower(varargin{1})
    ...
    otherwise       % Perform keyword search
        ...
        id2 = strcmp('-all', varargin);
        if any(id2)     % Search through all help text
            varargin(id2) = ''; % Remove '-all' from search words
            dat = look4Engine(datFile, 3, varargin{:});
        else            % Perform H1 line search
...
if createNew  % (Re)Create new database
    ...
    pathList = textscan_expand(path, '%s', 'delimiter', pathsep);
    % pathList = strread(path, '%s', 'delimiter', ';');
    w = cellfun(@what, pathList, 'UniformOutput', false);
    w = vertcat(w{:});
    fprintf('Done\n');
    ...
    p = fullfile(w.path, filesep);
    idM = find(strncmpi(p, dat(:, 1), length(p)));
    ...
    if ~isempty(idM)
        ...
        pp = fullfile(w.path, 'private', filesep);
        idP = find(strncmpi(pp, dat(:, 1), length(pp)));
        ...
        for iC = 1:length(w(1).classes)
            ...
            cp = fullfile(w(1).path, ['@', w(1).classes{iC}], filesep);
            idC = [idC; find(strncmpi(cp, dat(:, 1), length(cp)))];
            
            % Find PRIVATE directory under CLASS in the database
            pp = fullfile(w(1).path, ['@', w(1).classes{iC}], 'private', filesep);
            idP = [idP; find(strncmpi(pp, dat(:, 1), length(pp)))];
...
% bgcolor1 = [.85 .85 .85];
bgcolor1 = [212 208 200]/255;
bgcolor2 = [1 1 1];
bgcolor3 = [.7 .7 1];
fgcolor1 = [0 0 0];
fgcolor2 = [.5 .5 .5];

resultFieldFontSize = 12;
resultFieldFontWeight = 'bold';
helpFieldFontSize   = 12;
helpFieldFontWeight = 'bold';
...
figH = figure(                                                         ...
    ...
    'defaultuicontrolfontname'           , 'tahoma',                    ...
...
uicontrol(                                                       ...
    'style'               , 'listbox',                             ...
    'tag'                 , 'SearchResults',                       ...
    'position'            , [20 fr-40 fw-40 fh-fr-80],             ...
    'backgroundcolor'     , bgcolor2,                              ...
    'interruptible'       , 'off',                                 ...
    'fontname'            , 'FixedWidth',                          ...
    'fontsize'            , resultFieldFontSize,                   ...
    'fontweight'          , resultFieldFontWeight,                 ...
...
uicontrol(                                                       ...
    'style'               , 'listbox',                             ...
    'tag'                 , 'HelpText',                            ...
    'position'            , [20 20 fw-40 fr-90],                   ...
    'backgroundcolor'     , bgcolor2,                              ...
    'buttondownfcn'       , @activateKeyPressMode,                 ...
    'fontname'            , 'FixedWidth',                          ...
    'fontsize'            , helpFieldFontSize,                     ...
    'fontweight'          , helpFieldFontWeight,                   ...
    'horizontalalignment' , 'left',                                ...
    'enable'              , 'inactive',                            ...
    'visible'             , 'on');
    ...
    switch get(handles.look4MainGUI, 'selectiontype')
        ...
        case 'normal' % Single-click
            ...
            if ~isempty(tmp)
                ...
                tmp = textscan_expand(tmp, '%s', 'delimiter', '\n', ...
                    'whitespace', '');
            end
...
if isempty(result)
    ...
    if exist('startup.m', 'file')
        ...
        if strcmp(result.type, 'reset')
            ...
            p = lower(textscan_expand(path, '%s', 'delimiter', pathsep));
            [sel, ok] = listdlg(...
...
switch opt
    ...
    case 'modify'
        ...
        str = textscan_expand(fname, '%s', 'delimiter', '\n');
        [p, n] = fileparts(fname);
    ...
    case 'retrieve'
        ...
        if isempty(id)
            ...
            if exist(update_fname, 'file')
                ...
                str = textscan_expand(update_fname, '%s', 'delimiter', '\n');
                id = strmatch('% out.', str);
                if length(id) == 2
                    eval(str{id(1)}(3:end));
                    eval(str{id(2)}(3:end));
...
try
    ...
    s = textscan_expand(str, '%d', 'delimiter', ':')';
    if length(s) == 2
...
function out = textscan_expand(varargin)

try
    out1 = textscan(varargin{:});
    out = out1{1};
catch ME
    throw(ME)
end
