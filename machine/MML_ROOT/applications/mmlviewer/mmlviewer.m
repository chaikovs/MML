...
% Last Modified by GUIDE v2.5 17-Oct-2016 22:17:26
...
try
    ...
    % Make TangoNames the default, if possible
    Fields = get(handles.ListBox3, 'String');
    if length(Fields) >= 1
        k = find(strcmpi(Fields, 'TangoNames'));
        if isempty(k)
            k = find(strcmpi(Fields, 'TangoNames'));
        end    
        if ~isempty(k)
...
% Get data
try
    Data = getandplotdata(handles, Fields1{i}, Fields2{j}, DotField, family2dev(Fields1{i}));
catch
...
if isstruct(Data)
    ...
    if get(Box1, 'Value') > size(SubDataField,1)
        ...
        % Make TangoNames the default, if possible
        if length(SubDataField) >= 1
            k = find(strcmpi(SubDataField, 'TangoNames'));
            if ~isempty(k)
                set(Box1, 'Value', k);
            end
        end
    ...
    if strcmp(DataField, 'TangoNames')
