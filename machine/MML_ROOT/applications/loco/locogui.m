...
if nargin == 0  % LAUNCH GUI
    ...
    appdata = [];
    appdata.lastValidTag = 'uimenu_plotK';

    h149 = uimenu(...
        'Parent',h148,...
        'Callback','locogui(''uimenu_plotK_Callback'',gcbo,[],guidata(gcbo))',...
        'Label','Plot DK/K', ...
        'Tag','uimenu_plotK');
    ...
    appdata = [];
    appdata.lastValidTag = 'uimenu_exporK';

    h150 = uimenu(...
        'Parent',h148,...
        'Callback','locogui(''uimenu_exportK_Callback'',gcbo,[],guidata(gcbo))',...
        'Label','Export DK/K',...
        'Tag','uimenu_exporK');
...
Index = locoeditlist([(1:length(BPMData(StartFrom).HBPMIndex))' tmp(:)], 'BPMx', Checked);
Index = Index(:,1);
...
Index = locoeditlist([(1:length(BPMData(StartFrom).VBPMIndex))' tmp(:)], 'BPMz', Checked);
Index = Index(:,1);
...
Index = locoeditlist([(1:length(CMData(StartFrom).HCMIndex))'  CMData(StartFrom).HCMIndex(:)], 'HCOR', Checked);
Index = Index(:,1);
...
Index = locoeditlist([(1:length(CMData(StartFrom).VCMIndex))'  CMData(StartFrom).VCMIndex(:)], 'VCOR', Checked);
Index = Index(:,1);
        ...
        case 16
            ...
            if isfield(LocoFlags(i),'Method') && isfield(LocoFlags(i).Method,'DeltaChi2') && isfield(LocoFlags(i).Method.DeltaChi2,'FitParameter') && ~isempty(LocoFlags(i).Method.DeltaChi2.FitParameter)
                ...
                if 1
                    ...
                    ylabel(H_Axes,'\Delta \chi^2', 'FontSize', 14); grid on
                else
                ...
                axis tight;
                d = .08;
                text(.03,.9-0*d, sprintf('\\Delta \\chi^2(All BPMs) = %.1e',          LocoFlags(i).Method.DeltaChi2.BPMGroup),          'FontSize', 10, 'Units','Normalized');
                text(.03,.9-1*d, sprintf('\\Delta \\chi^2(All CMs)  = %.1e',          LocoFlags(i).Method.DeltaChi2.CMGroup),           'FontSize', 10, 'Units','Normalized');
                text(.03,.9-2*d, sprintf('\\Delta \\chi^2(All FitParameters) = %.1e', LocoFlags(i).Method.DeltaChi2.FitParameterGroup), 'FontSize', 10, 'Units','Normalized');
            else
...
% Start addon Laurent
function varargout = uimenu_plotK_Callback(h, eventdata, handles, varargin)
...
% If the LocoModel does not exist, then compute it
if isempty(LocoModel(i).M) | isempty(LocoModel(i).Eta) | isempty(LocoModel(i).ChiSquare)
    [LocoModel(i).M, LocoModel(i).Eta, LocoModel(i).ChiSquare] = getmodelresponsematrix(BPMData(i), CMData(i), FitParameters(i), ...
LocoFlags(i), LocoMeasData, RINGData);
    FileName = get(findobj(gcbf,'Tag','FileName'),'Userdata');
    save(FileName, 'LocoModel', 'FitParameters', 'BPMData', 'CMData', 'RINGData', 'LocoMeasData', 'LocoFlags');
    setappdata(gcbf, 'LocoDataCell', {BPMData, CMData, LocoMeasData, LocoModel, FitParameters, LocoFlags, RINGData});
end
...
figure
[K1, Tune1] = locodata({BPMData, CMData, LocoMeasData, LocoModel,FitParameters, LocoFlags, RINGData}, i-1, 'FitValues',[],'Tune',[]);
[K0, Tune0] = locodata({BPMData, CMData, LocoMeasData, LocoModel, FitParameters, LocoFlags, RINGData},   0, 'FitValues',[],'Tune',[]);

% count number of quadrupoles, 2 parameters to fit and K
quadnum = 0;
for k=1:length(FitParameters(1).Params),
    if size(FitParameters(1).Params{k},2) == 2 && strcmpi(FitParameters(1).Params{1}(1).FieldName,'K')
        quadnum = quadnum+1;
    end
end

if ~isempty(K1) & ~isempty(K0)
    if length(K1) > 10 % individual quads
        bar((K1(1:quadnum)-K0(1:quadnum))./K1(1:quadnum)*100, 'b');
        xlabel('Quadrupole number');
        ylabel(sprintf('(K(%d)-K(0))/K0 [%%]',i-1));
        title(texlabel(sprintf('DK (nu_x(%d)=%.4f, nu_x(0)=%.4f) (nu_y(%d)=%.4f, nu_y(0)=%.4f)', ...
            i-1, Tune1(1),Tune0(1), i-1, Tune1(2),Tune0(2))));
        xaxis([0 length(K1(1:quadnum))+1]); grid on; hold on;
        a = axis;
        plot([9 9], [-8 8],'r-.')
        plot([17 17], [-8 8],'r-.')
        plot([25 25], [-8 8],'r-.')
        ...
        if quadnum == 163,  % Nanoscopium
            quadpos = [getspos('Q1'); getspos('Q2') ; getspos('Q3') ; getspos('Q4'); ...
                getspos('Q5') ; getspos('Q6') ; getspos('Q7') ; getspos('Q8'); ...
                getspos('Q9'); getspos('Q10'); getspos('Q11'); getspos('Q12')];
        end

        [y id] = sort(quadpos);
        figure;
        h(1) = subplot(5,1,(1:4));
        DKoK = (K1(1:quadnum)-K0(1:quadnum))./K0(1:quadnum)*100;
        stem(y,DKoK(id));
        grid on;
        ylabel(sprintf('(K(%d)-K(0))/K0 [%%]',i-1));
        h(2) = subplot(5,1,5);
        ...
        % For QT
        B = FitParameters(end).Params(quadnum+1:end);
        QTi = cell2mat(cellfun(@(x) x.ElemIndex, {B{:}}, 'uni', false));
        
        QTiAll = family2atindex('QT');
        l_=zeros(length(QTi),1);
        for ik=1:length(QTi),            
            l_(ik) = find(QTiAll-QTi(ik) == 0);            
        end
        QTdevAll = family2dev('QT', 0);
        QTdev = QTdevAll(l_,:);
        
        figure
        h(1) = subplot(5,1,(1:4));
        bar(getspos('QT',QTdev), physics2hw('QT', 'Monitor', K1(quadnum+1:end),QTdev), 'b');
        title('QT value');
        ylabel('Current (A)')
        h(2) = subplot(5,1,5);
        drawlattice
        ...
        bar((K1-K0)./K0*100, 'b');
        xlabel('Quadrupole family number');
        ylabel(sprintf('(K(%d)-K(0))/K0 [%%]',i-1));
        title(texlabel(sprintf('DK (nu_x(%d)=%.4f, nu_x(0)=%.4f) (nu_y(%d)=%.4f, nu_y(0)=%.4f)', ...
            i-1, Tune1(1),Tune0(1), i-1, Tune1(2),Tune0(2))));
        xaxis([0 length(K1)+1]); grid on; hold on;
    end
end

% exporting data into matlab workspace
function varargout = uimenu_exportK_Callback(h, eventdata, handles, varargin)
...
if ~isempty(K1) & ~isempty(K0)
    LOCOstruct.DKoK = (K1-K0)./K0*100;
    LOCOstruct.DK = (K1-K0);
    LOCOstruct.CreatedBy = 'locogui';
    LOCOstruct.TimeStamp = datestr(now)
end

assignin('base', 'LOCOstruct', LOCOstruct);
% End addon Laurent
