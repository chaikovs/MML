...
ButtonNameG = questdlg('Gains and Coupling?','LOCO STARTING POINT','Use Present Setting','Nominal Gain=1, Coupling=0','Use Present Setting');
switch ButtonNameG,
...
if GuessIt
    ...
    switch ButtonName,
        case 'Get From File'
            ...
            try
                 load([BPMRespDirectory BPMRespFile], 'MachineConfig','Rmat');
            catch err
                fprintf('   Lattice configuration not found in the response matrix file.  You don''t need it but it''s nice to have.\n');
            end
            
            %Get family from RespFile
            BPMxFamily=Rmat(1,1).Monitor.FamilyName;
            BPMyFamily=Rmat(2,2).Monitor.FamilyName;
            HCMFamily =Rmat(1,1).Actuator.FamilyName;
            VCMFamily =Rmat(2,2).Actuator.FamilyName;
            % clear Rmat before getrespmat
            clear Rmat;
...
switch ButtonNameG,
    ...
    otherwise
        ...
        h = waitbar(0,sprintf('Computing: %2d %%',0), 'Name', 'Building loco input');
        
        Nh = length(Rmat(1,1).Monitor.DeviceList);
        Nv = length(Rmat(2,2).Monitor.DeviceList);
        Nsize = Nh + Nv;
        ...
        for i = 1:length(Rmat(1,1).Monitor.DeviceList)
            ...
            waitbar(i/Nsize, h, sprintf('Computing progression: %3d %%', floor(i/Nsize*100)));
        end
        ...
        for i = 1:length(Rmat(2,2).Monitor.DeviceList)
            ...
            waitbar((i+Nh)/Nsize, h, sprintf('Computing progression: %3d %%', floor((i+Nv)/Nsize*100)));
        end
        close(h);
end
...
if ~GuessIt
    if exist('buildlocofitparameters', 'file')
        buildlocofitparameters(OutputFileName, THERING);
    else
