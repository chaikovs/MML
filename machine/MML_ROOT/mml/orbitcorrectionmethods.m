function [OCS, SmatNoWeights, S, U, V] = orbitcorrectionmethods(OCS, Smat, S, U, V)
...
%  Addition for Soleil by Laurent S. Nadolski
%    Constraints on min and max RF frequency variations
...
% Machine dependent
if strcmpi(getfamilydata('SubMachine'),'StorageRing') && strcmpi(getfamilydata('Machine'),'SOLEIL')
    DeltaRFmin = 1e-7;   % MHz minimum variation allowed by Master oscillator
    DeltaRFmax = 100e-6; % MHz maximum variation allowed in one step
else
    DeltaRFmin = 1e-17;   % MHz minimum variation allowed by Master oscillator
    DeltaRFmax = 100e6; % MHz maximum variation allowed in one step
end
...
for i = 1:length(OCS.CM)
    if ~isfield(OCS, 'CMWeight') || isempty(OCS.CMWeight{i})
        % When using all singular values this does not do anything
...
elseif any(OCS.FitRF == [1 2 3 4 5])
    ...
    RFWeight = 10 * mean(std(Smat)) / std(OCS.Eta);
    Smat = [Smat RFWeight*OCS.Eta];
else
...
if ~isempty(i)
    warning('Requested number of singular values is greater than the total.  Removing %d values.',length(i));
...
if OCS.FitRF == 2 || OCS.FitRF == 3
    ...
    for i = 1:length(OCS.CM)
        if ismemberof(OCS.CM{i}.FamilyName, 'HCOR')
            ...
            if strcmpi(OCS.CM{i}.Units, 'Hardware')
                % PhysicsUnitsScaling = (HCM.Data./OCS.CM{i}.Data);  %This does not work for zero setpoints
                PhysicsUnitsScaling = hw2physics(OCS.CM{i}. FamilyName,OCS.CM{i}.Field, 1, OCS.CM{i}.DeviceList); 
            else
                PhysicsUnitsScaling = 1;
            end
            HCMEnergyChangeScalar = -1 * PhysicsUnitsScaling .* OCS.CM{i}.Dispersion / getmcf / L;
...
elseif OCS.FitRF == 4
    % Constrained L.S. with SVD
    % Constraint: dot(DispersionX,   Smat * dHCM) = 0
    %         or  dot(DispersionX' * Smat,  dHCM) = 0 or total dot(DisperionX, Smat * HCM)
    R = [];
    DispDotSmatHCM = 0;
    for i = 1:length(OCS.BPM)
...
if OCS.FitRF
    ...
    OCS.DeltaRF = RFWeight * Delcm(end);
    if strcmpi(getfamilydata('SubMachine'), 'StorageRing')
        % Begin Add by Laurent % To be done with variables
        % test for avoiding to move too often RF frequency
        %fprintf('Prog %e \n',DelRF);
        if abs(OCS.DeltaRF) < DeltaRFmin % minimum RF is .1 Hz 
            OCS.DeltaRF = 0;
        end
        if abs(OCS.DeltaRF) > DeltaRFmax % maximum is 100 Hz
            warningText = sprintf('RF change too large DeltaRF = %f Hz (> %f Hz) \nRF frequency shift has to be fixed first by hand.\nSee expert steprf command', ...
                hw2physics('RF','Setpoint',OCS.DeltaRF), hw2physics('RF','Setpoint',DeltaRFmax));
            if strcmpi(getmode('RF'), 'Simulator')
                warning(warningText);
            else
                error(warningText);
            end
...
% Predict Orbit
if OCS.FitRF
    OrbitDeltaTotal = [SmatNoWeights OCS.Eta] * [Delcm; OCS.DeltaRF];
else
