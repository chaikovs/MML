function [Dx, Dy, Sx, Sy] = modeldisp(varargin)
...
if isempty(UnitsFlag)
    UnitsFlag = 'Physics';
%     if strcmpi(Family1, 'All') | strcmpi(Family2, 'All')
%         UnitsFlag = 'Physics';
%     else
...
d(1).Actuator = getsp('RF','Physics','Struct');
...
if DisplayFlag
    ...
    % plotdisp cannot handle a AT family name or 'All'
    % so a new dispersion plot was written below
    %plotdisp(d);

    UnitsFlag = d(1).Units;
    UnitsString = d(1).UnitsString;

    if strcmp(UnitsString, 'mm/MHz')
        TitleString = sprintf('Change in Orbit / Change in RF (Delta RF=%g Hz)', DeltaRF);
    elseif strcmp(UnitsString, 'meters/(dp/p)') | strcmp(UnitsString, 'm/(dp/p)')
        TitleString = sprintf('Dispersion Function: %s  (\\alpha=%.2e, f_{RF}=%f MHz, \\Delta f_{RF}=%g Hz)', texlabel('-alpha f_{RF} {Delta}Orbit / {Delta}f_{RF}'), MCF, CavityFrequency*1e-6, DeltaRF);
    else
        TitleString = 'Dispersion Function';
    end
