...
elseif strcmp(Family, 'TwissData')
    ...
    if isfield(AT, 'ATParameterGroup')
        ...
        for i = 1:1, %size(NewSP,1), % Not working for family but OK for sliced element
        ...
        if any(strcmpi(AT.ATType, {'HCM', 'CH', 'HCOR', 'FHCOR', 'Kicker'}))
            ...
            for i = 1:size(NewSP,1)
                ...
                if isfield(THERING{ATIndexList(i)}, 'Roll')
                    ...
                    fprintf('setpvmodel: Roll not found\n');
                    Roll = [getroll(Family, Field, DeviceList(i,:))  0];
                    VCMDevList = family2dev('VCOR');
                    iVCM = findrowindex(DeviceList(i,:), VCMDevList);
                    if ~isempty(iVCM)
                      try
                           ATIndexVCM = family2atindex('VCOR', DeviceList(i,:));
                           if ATIndexVCM == ATIndexList(i)
                               Roll = [Roll(1) getroll('VCOR', Field, DeviceList(i,:))];
                           else
                                Roll = [0 0];
                           end
                       catch
        ...
        elseif any(strcmpi(AT.ATType, {'VCM', 'CV', 'VCOR', 'FVCOR'}))
            ...
            for i = 1:size(NewSP,1)
                ...
                if isfield(THERING{ATIndexList(i)}, 'Roll')
                    ...
                    HCMDevList = family2dev('HCOR');
                    iHCM = findrowindex(DeviceList(i,:), HCMDevList);
                    if ~isempty(iHCM)
                       try
                           ATIndexHCM = family2atindex('HCOR', DeviceList(i,:));
                           if ATIndexHCM == ATIndexList(i)
                               Roll = [getroll('HCOR', Field, DeviceList(i,:)) Roll(2)];
                           else
                                Roll = [0 0];
                           end
                       catch
