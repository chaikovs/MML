...
% Last Modified by GUIDE v2.5 30-Mar-2007 16:47:50
...
try
    ...
    set(handles.('FileName'),'Position', [p(1) .8 p(3) p(4)]);   % Characters
...
if isempty(h)
    cla(handles.Graph1);
    plot(handles.Graph1, sx, Data3,'.-r');
    hold(handles.Graph1, 'on');
    plot(handles.Graph1, sx, Data1,'.-b');
    %bar(handles.Graph1, sx, Data1);
...
%set(handles.('Trace1RMS'),'String',sprintf('Trace 1 (%.4e rms)', std(Data1)));   % STD
%set(handles.('Trace1RMS'),'String',sprintf('Trace 1 (%.4e rms)', (length(Data1)-1)*std(Data1)/length(Data1)));  % RMS
...
% if UnitsFlag == -1
%     set(handles.('DataList1'),'String', [BPMxMAT, num2str(Data1(:),'%+10.1f')]);
%     set(handles.('Trace1RMS'),'String',sprintf('Trace 1 (%.1f rms)', std(Data1)));
% elseif UnitsFlag == 1
%     set(handles.('DataList1'),'String', [BPMxMAT, num2str(Data1(:),'%+.4e')]);
%     set(handles.('Trace1RMS'),'String',sprintf('Trace 1 (%.4e rms)', std(Data1)));
% else
%     set(handles.('DataList1'),'String', [BPMxMAT, num2str(Data1(:),'%+7.4f')]);
%     set(handles.('Trace1RMS'),'String',sprintf('Trace 1 (%.4f rms)', std(Data1)));
% end
...
if isempty(h)
    cla(handles.Graph2);
    plot(handles.Graph2, sy, Data4,'.-r');
    hold(handles.Graph2, 'on');
    plot(handles.Graph2, sy, Data2,'.-b');
...
% Set the list box header
%set(handles.('Trace2RMS'),'String',sprintf('Trace 1 (%.4e rms)', std(Data2)));  % STD
%set(handles.('Trace2RMS'),'String',sprintf('Trace 1 (%.4e rms)', (length(Data2)-1)*std(Data2)/length(Data2)));  % RMS
%set(handles.Trace2RMS,'String',sprintf('%.4e mean, %.4e rms', mean(Data2), (length(Data2)-1)*std(Data2)/length(Data2)));  % RMS
i = find(~isnan(Data2));
if isempty(i)
    MeanString = '';
    RMSString  = '';
...
% if UnitsFlag == -1
%     set(handles.('DataList2'),'String', [BPMyMAT, num2str(Data2(:),'%+10.1f')]);
%     set(handles.('Trace2RMS'),'String',sprintf('Trace 1 (%.1f rms)', std(Data2)));
% elseif UnitsFlag == 1
%     set(handles.('DataList2'),'String', [BPMyMAT, num2str(Data2(:),'%+.4e')]);
%     set(handles.('Trace2RMS'),'String',sprintf('Trace 1 (%.4e rms)', std(Data2)));
% else
%     set(handles.('DataList2'),'String', [BPMyMAT, num2str(Data2(:),'%+7.4f')]);
%     set(handles.('Trace2RMS'),'String',sprintf('Trace 1 (%.4f rms)', std(Data2)));
% end
...
%FileName = getfamilydata('Default', 'BPMFile');
...
% Graph 2
try
    DataStruct = getdata(DataStruct2.FamilyName, DataStruct1.DeviceList, [DirectoryName FileName], 'Struct');
    if UnitMenu == 1 && strcmpi(DataStruct.Units,'Hardware')
        % Physics units requested
        DataStruct = hw2physics(DataStruct);
    elseif (UnitMenu==2 || UnitMenu==3) && strcmpi(DataStruct.Units,'Physics')
...
FileNameX = getappdata(handles.figure1, 'FileName');
...
if ~isempty(FileNameX)
    ...
    %set(handles.FileName,'String', sprintf('File 1: %s', FileNameX));
    set(handles.FileName,'String', sprintf('%s', FileNameX));
else
    set(handles.FileName,'String', 'File 1: no data');
end
...
if strcmpi(FileNameX, FileNameY) && ~isempty(FileNameX)
    ...
    p = get(handles.FileName,'Position');
    %set(handles.FileName,'Position', [p(1) 4 p(3) p(4)]);   % Points
    set(handles.FileName,'Position', [p(1) .4 p(3) p(4)]);   % Characters
    p = get(handles.SaveTime,'Position');
    %set(handles.SaveTime,'Position', [p(1) 16.5 p(3) p(4)]);  % Points
    set(handles.SaveTime,'Position', [p(1) 1.6 p(3) p(4)]);    % Characters
else
    ...
    set(handles.FileName,'Position', [p(1) 1.1 p(3) p(4)]);  % Characters
...
% a = axis(handles.Graph1);
% axis(handles.Graph1, [AxisRange1X(1) AxisRange1X(2) a(3) a(4)]);
% a = axis(handles.Graph3);
% axis(handles.Graph3, [AxisRange1X(1) AxisRange1X(2) a(3) a(4)]);
% 
% 
% % Set x-axis for everybody else
% set(handles.Graph2,      'XLim', [AxisRange1X(1) AxisRange1X(2)]);
% set(handles.Graph4,      'XLim', [AxisRange1X(1) AxisRange1X(2)]);
% set(handles.LatticeAxes, 'XLim', [AxisRange1X(1) AxisRange1X(2)]);
...
p = get(handles.FileName, 'Position');
set(handles.FileName, 'Position',  [p(1) p(2) 19 p(4)]);
p = get(handles.FileNameY, 'Position');
