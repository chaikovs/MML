...
for i = length(varargin):-1:1
    ...
    elseif strcmpi(varargin{i},'manual')
        ModeFlag = varargin(i);
        varargin(i) = [];
    elseif strcmpi(varargin{i},'NoSP')
        SPFLAG = 0;
        varargin(i) = [];
    end        
end
...
if isempty(DeltaTune)
    ...
    DeltaTune(1,1) = str2double(answer{1});
    DeltaTune(2,1) = str2double(answer{2});
    fprintf('Tune variation wanted is dnux=%f dnuz=%f\n',DeltaTune(1,1), DeltaTune(2,1))
    
    if strcmpi(family2mode('BEND'), 'Online')
        if any(abs(DeltaTune)>DeltaTuneMax)
            warndlg('Too large. Action canceled')
            fprintf('Too large. Action canceled (max value is %.2f)\n',DeltaTuneMax)
            return;
        end
    end
end
...
% Here so that  ActuatorFamily is defined
if DeltaTune(1)==0 && DeltaTune(2)==0,
    DelQuad = zeros(size(ActuatorFamily))';
    return
end
