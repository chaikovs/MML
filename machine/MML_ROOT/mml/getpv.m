...
if FamilyIndex 
    ...
    ExtraTimeDelay = etime(clock, t0);
    if StructOutputFlag || nargout >= 3
        [AM, tout, DataTime, ErrorFlag, DeviceIndex] = local_getpv(AO, Field, DeviceList, OutputDataType, t-ExtraTimeDelay, FreshDataFlag, TimeOutPeriod, InputFlags{:});

    
    else
        [AM, tout] = local_getpv(AO, Field, DeviceList, OutputDataType, t-ExtraTimeDelay, FreshDataFlag, TimeOutPeriod, InputFlags{:});
    end
...
% June 18, 2013. Commented not usefull
% if StructOutputFlag || nargout >= 3
%     % .DataTime is the time on computer taking the data but
%     % reference it w.r.t. the time zone where Matlab is running
%     DateNumber1970 = 719529;  %datenum('1-Jan-1970');
...
if ErrorFlag
    ...
    if size(FamilyTest, 1) == 1
        ...
        [AM, tout, DataTime, ErrorFlag] = getpv(FamilyTest, Field, DeviceList, t, FreshDataFlag, TimeOutPeriod, 'Numeric', varargin{:});
    else
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [AM, tout, DataTime, ErrorFlag, DeviceIndex] = local_getpv(AO, Field, DeviceList, OutputDataType, t, FreshDataFlag, TimeOutPeriod, varargin)
%  INPUTS
...
if ~isfield(AO, Field);
    ...
    if strcmpi(Mode,'Simulator') || strcmpi(Mode,'Model')
        ExtraTimeDelay = etime(clock, t0);
        [AM, tout, DataTime, ErrorFlag] = getpvmodel(AO.FamilyName, Field, DeviceList, t-ExtraTimeDelay, 'Physics', 'Numeric', InputFlags{:});
        tout = tout + ExtraTimeDelay;
