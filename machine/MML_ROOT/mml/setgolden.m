...
%
%  EXAMPLES
%  1. [BPMxGolden BPMZGolden] = setgolden;
%
%  See Also savegoldenorbit, getgolden, plotgoldenorbit
...
if nargin == 0
    ...
    [FileName, DirName] = uigetfile([DirName,filesep,'BPMGolden*.mat'], 'Select a data file:');
...
if nargin >= 3
    ...
    elseif isnumeric(varargin{3})
        ...
        if ~isfield(varargin{1}, 'DeviceList')
            % Laurent ligne commente la suivante
            % varargin{1}.DeviceList = varargin{3};
            % Laurent ligne suivante
...
if ~isempty(FileName)
    if isempty(Family)
        ...
        if length(Family)> 2
            warning('Too many families (2 expected), skip last ones')
            Family(3:end)=[];
        end
        if isempty(Family)
