...
%
%  OPTIONAL
%  1. 'BPMCOUPLING' - Correct for BPM gains and coupling (from LOCO) only if 'Online measurementà
%
%  OUTPUT
...
%
%  Written by William J. Corbett and Gregory J. Portmann
%  Modified by Laurent S. Nadolski
%  More generic, Eta option was not working
%  March 2009, Add option for BPM coupling ang gain
...
for i = length(varargin):-1:1
    ...
    elseif strcmpi(varargin{i},'NoCoupling')
        CouplingFlag = 0;
        varargin(i) = [];    
    elseif strcmpi(varargin{i},'Coupling')
        CouplingFlag = 1;
        varargin(i) = [];    
    elseif strcmpi(varargin{i},'Hardware')
        UnitsFlag = varargin(i);
        varargin(i) = [];    
    elseif strcmpi(varargin{i},'BPMCoupling')
        CouplingFlag = 1;
        varargin(i) = [];    
    elseif strcmpi(varargin{i},'NoBPMCoupling')
        CouplingFlag = 0;
        varargin(i) = [];    
    end
end
...
if isstruct(Dx)
    ...
    if isfamily(Dx.Monitor.FamilyName)
        ...
        X1LabelString = sprintf('%s Position [m]', Dx.Monitor.FamilyName);
    elseif strcmpi(Dx.Monitor.FamilyName, 'all');
        global THERING
        sx = findspos(THERING, 1:length(THERING)+1);
        X1LabelString = 'Position [m]';
    else
        sx = 1:length(Dx.Data);
    ...
    if isfamily(Dy.Monitor.FamilyName)
        ...
        X2LabelString = sprintf('%s Position [m]', Dy.Monitor.FamilyName);
    elseif strcmpi(Dy.Monitor.FamilyName, 'all');
        global THERING
        sy = findspos(THERING, 1:length(THERING)+1);
        X2LabelString = 'Position [m]';
    else
        sy = 1:length(Dy.Data);
    ...
    if strcmpi(UnitsFlag,'Physics')
        ...
        if ~isempty(RF0) && ~isempty(MCF)
            TitleString = sprintf('Dispersion Function: %s  (\\alpha=%3.2e, f=%f)', texlabel('-alpha f {Delta}Orbit / {Delta}f'), MCF, RF0);
...
% Plot
clf reset
%set(gcf,'NumberTitle','off','Name','Dispersion ');
%set(gcf,'Name','Dispersion ');

if CouplingFlag && strcmpi(DxOut.Mode, 'Online')
    % get BPM gains and coupling for LOCO file nominal
    Data = family2coupling(DxOut.Monitor.DeviceList);
    for ik =1:size(DxOut.Monitor.DeviceList,1),
        eta = deal(Data.Cinv{ik}(:,:)*[Dx(ik); Dy(ik)]);
        Dx(ik) = eta(1);  Dy(ik) =  eta(2);
    end
    DxOut = Dx;
    DyOut = Dy;
end
