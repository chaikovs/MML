...
WaitFlag = 4;
MCF = [];
BPMxFamily = 'BPMx'; %gethbpmfamily;  % Just an extra monitor
BPMyFamily = 'BPMz'; %getvbpmfamily;  % Just an extra monitor
StructOutputFlag = 0;
FileName = -1;
ArchiveFlag = 0;
DisplayFlag = -1;
...
% Check DeltaRF for reasonable values
maxRFchange = 1000;% Hz
if strcmpi(RFUnitsString, 'MHz')
    if abs(max(DeltaRF)-min(DeltaRF)) > maxRFchange*1e-6  % .001 MHz
        tmp = questdlg(sprintf('%f MHz is a large RF change.  Do you want to continue?', abs(max(DeltaRF)-min(DeltaRF))),'Chromaticity Measurement','YES','NO','YES');
        if strcmp(tmp,'NO')
            Chromaticity=[];
            return
        end
...
elseif strcmpi(RFUnitsString, 'kHz')
    if abs(max(DeltaRF)-min(DeltaRF)) > maxRFchange*1e-3  % kHz
        tmp = questdlg(sprintf('%f kHz is a large RF change.  Do you want to continue?', abs(max(DeltaRF)-min(DeltaRF))),'Chromaticity Measurement','YES','NO','YES');
        if strcmp(tmp,'NO')
            Chromaticity=[];
            return
        end
...
elseif strcmpi(RFUnitsString, 'Hz')
    if abs(max(DeltaRF)-min(DeltaRF)) > maxRFchange  % Hz
        tmp = questdlg(sprintf('%f Hz is a large RF change.  Do you want to continue?', abs(max(DeltaRF)-min(DeltaRF))),'Chromaticity Measurement','YES','NO','YES');
        if strcmp(tmp,'NO')
            Chromaticity=[];
            return
        end
...
c.Monitor = gettune('Struct', ModeFlag);     % Should WaitFlag before to be sure to get a good measurement ?????
c.Actuator = RFsp;
