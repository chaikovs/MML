...
device_name.scan = 'lt1/dg/fae-scan-v2'
device_name.fente = 'lt1/dg/fae-fente'
%device_name.scan1D = 'lt1/dg/fae-scan1D-v2'
...
actuator_attr_names=tango_read_attribute(dev,'actuatorsDataList');
vectx=tango_read_attributes(dev,actuator_attr_names.value);
fonction_error;
xdata = vectx(1).value;
...
sensors_attr_names=tango_read_attribute(dev,'sensorsDataList');
...
mc_val_list = tango_read_attributes(dev, sensors_attr_names.value);
%- always check error
if (tango_error == -1)
  %- handle error 
...
ydatamc1 = mc_val_list(2).value;
%- store 'Sensor2Data' value in local variable
ydatamc2 = mc_val_list(1).value;
...
% modification version scan : plus d'attribut measureDate. février 08
traitement_date =  datestr(datenum(now),'dd-mmm-yyyy HH:MM:SS')
...
set(handles.horodatage_edit1,'String',traitement_date);
setappdata(handles.figure1,'Date',traitement_date);
