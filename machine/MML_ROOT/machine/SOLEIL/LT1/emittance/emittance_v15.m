...
setappdata(handles.figure1, 'G',0.038);
...
device_name.calibration = 'lt1/dg/emit-calib';
...
set(handles.G_edit12,'String',num2str(38));
clear('figh');
...
setappdata(handles.figure1,'posl2Gmax',35130); %35130
...
dev=device_name.lentille_haute;
edit_butee_haute = handles.LHBH_edit25;
edit_butee_basse = handles.LHBB_edit24;

test_butee_axis(dev,edit_butee_haute,edit_butee_haute)
                            %errorstatus=tango_command_inout(dev,'AxisGetErrorStatus');
                % Blimitswitch.value = tango_read_attribute(dev,'backwardLimitSwitch');
...
edit_butee_haute = handles.LBBH_edit26;
edit_butee_basse = handles.LBBB_edit27;
test_butee_axis(dev,edit_butee_haute,edit_butee_haute)
                % errorstatus=tango_command_inout(dev,'AxisGetErrorStatus');
                % 
                % if (tango_error == -1)
...
str1=tango_read_attribute(dev1,'position');
str2=tango_read_attribute(dev2,'position');
...
pos1=tango_read_attribute(device_name.lentille_haute,'position');
set(handles.pos1_edit29,'String',num2str(pos1.value(1)));
pos2=tango_read_attribute(device_name.lentille_basse,'position');
...
dev=device_name.calibration; % modification 10 oct 07
...
if isequal(tango_error,-1)
    ...
    if res.value>996.0
        ...
        tango_read_attribute(dev,'xMagFactor'); % modification 10 oct 07
...
% % %         val=sprintf('%2.1f',toto)
        ...
        toto=tango_read_attribute(dev,'correctedImage'); % modification 10 oct 07
        figure(2);
        imagesc(toto.value);
        colormap(gray);
...
if (tango_error == -1)
    ...
    % errorstatus=4  limitswitch backward
     ...
     if ~Blimitswitch.value    
        ...
        while strcmp(tango_command_inout(dev,'State'),'MOVING')
            set(handles.fondGmin_pushbutton31,'BackgroundColor','red');
            pause(0.2);
            set(handles.fondGmin_pushbutton31,'BackgroundColor',[0.979 1.0 0.822]);
            pause(0.2);
...
%             pause(0.5);
    ...
    tango_command_inout(dev,'DefinePosition',0);
...
%             set(handles.tata_radiobutton10,'BackgroundColor',[0.702 0.702 0.702]);
    ...
    tango_write_attribute(dev,'position',posl2Gmin);
...
Flimitswitch = tango_read_attribute(dev,'forwardLimitSwitch');
%           errorstatus=tango_command_inout(dev,'AxisGetErrorStatus');
% Blimitswitch.value = Blimitswitch.value.value
% Flimitswitch.value = Flimitswitch.value.value
...
%             set(handles.Lenthaute_radiobutton7,'BackgroundColor',[0.702 0.702 0.702]);
    ...
    tango_write_attribute(dev,'position',posl1Gmoy);
    fonction_error
    pause(8)
...
%             set(handles.tata_radiobutton10,'BackgroundColor',[0.702 0.702 0.702]);
    ...
    % avancer du nombre de pas exact correspondant au grandissement
    tango_write_attribute(dev,'position',posl2Gmoy);
    pause(8)
    temporisation_mvt_axis(dev);
...
if (tango_error == -1)
 ...
 %   if ~Blimitswitch.value                  % cas ou l'axe n'est pas en butee backward (en haut)

        if Flimitswitch.value                % si on est en but�e basse forward on �teind le voyant
            set(handles.LHBB_edit24,'BackgroundColor',[0.702 0.702 0.702]);
        end

        tango_command_inout(dev,'InitializeReferencePosition');        % aller en butee haute
        pause(0.5)
                    % on attent que l'axe se mette en mouvement
                    %temporisation_mvt_axis(dev);
                    %         if (temporisation_mvt_axis(dev) == -1)
                    %          % abort
                    %         end
        while strcmp(tango_command_inout(dev,'State'),'MOVING') % attendre la fin de la commande
            set(handles.fondGmax_pushbutton29,'BackgroundColor','red');
            pause(0.2);
            set(handles.fondGmax_pushbutton29,'BackgroundColor',[0.979 1.0 0.822]);
            pause(0.2);
            % on affiche la position de la lentille
            pos1=tango_read_attribute(dev,'position');
            set(handles.pos1_edit29,'String',num2str(pos1.value(1)));
        end

        pos1=tango_read_attribute(dev,'position');  % on affiche la position de la lentille
        set(handles.pos1_edit29,'String',num2str(pos1.value(1)));
 %   end


    set(handles.LHBH_edit25,'BackgroundColor','red');   % indiquer la butee haute sur moteur 1 (lentille haute)
    ...
    if strcmp(ms,'ALARM')|strcmp(ms,'STANDBY')
        tango_write_attribute(dev,'position',posl1Gmax);
        disp('ca y va')
    else
        tango_print_error_stack;
        %break;
    ...
    pause(2);
    set(handles.LHBH_edit25,'BackgroundColor',[0.702 0.702 0.702]);     % eteindre le voyant but�e haute
...
if (tango_error == -1)
 ...
 %   if ~Blimitswitch.value    % cas ou l'axe n'est pas en butee backward (en haut)
        ...
        tango_command_inout(dev,'InitializeReferencePosition');         % aller en butee haute backward
        pause(0.5)
       ...
       while strcmp(tango_command_inout(dev,'State'),'MOVING')              % attendre la fin de la commande
            ...
            pause(0.2);
            % on affiche la position de la lentille
            pos2=tango_read_attribute(dev,'position');
            set(handles.pos2_edit30,'String',num2str(pos2.value(1)));
        end
        pos2=tango_read_attribute(dev,'position');              % on affiche la position de la lentille
        set(handles.pos2_edit30,'String',num2str(pos2.value(1)));
   % end

    set(handles.LBBH_edit26,'BackgroundColor','red');   % indiquer la butee haute sur moteur 1 (lentille haute)
    ...
    ms = tango_command_inout(dev,'State')
    if strcmp(ms,'ALARM')|strcmp(ms,'STANDBY')
        tango_write_attribute(dev,'position',posl2Gmax);
        disp('ca y va')
    else
        tango_print_error_stack;
        %break;
    ...
    set(handles.LBBH_edit26,'BackgroundColor',[0.702 0.702 0.702]);         % eteindre le voyant rouge butee backward s'il est allum�
...
if isequal(logdev1,0)&isequal(logdev2,0)
    ...
    if (tango_error == -1)
        ...
        if ~Flimitswitch.value
            ...
            postrans = tango_read_attribute(dev,'position');
            tango_write_attribute(dev,'position',postrans + nbpas);
...
%             
%             % on bouge
%             tango_write_attribute(dev,'position',numeropas);
%         
%             % temporisation : 
%             temporisation_mvt_axis(dev);
%             
...
device_name = getappdata(handles.figure1,'device_name');
dev = device_name.videograbber;
dev1 = device_name.calibration ; % modification 10 oct 07
...
if isequal(logdev1,0)&isequal(logdev2,0)
    ...
    if (tango_error == -1)
        ...
        if ~isequal(toto,{})
            tango_command_inout(dev,'DefinePosition',str2num(toto{:}));
            fonction_error;
...
%         errordlg(num2str(res),'AxisGetMotionStatus');
        ...
        if ~isequal(toto,{})
            ...
            posref=tango_read_attribute(dev,'position');
            fonction_error;
            Blimitswitch = tango_read_attribute(dev,'backwardLimitSwitch');
            Flimitswitch = tango_read_attribute(dev,'forwardLimitSwitch');
            ...
            tango_write_attribute(dev,'position',str2num(toto{:}));
            % on attent que l'axe se mette en mouvement
            if (temporisation_mvt_axis(dev) == -1)
             % abort 
            end
