...
for ii = 1:size(DeviceList,1)
    ...
    %
    if exist('PhysConstant', 'file'),
        phc = PhysConstant;
        E0 = phc.natural_unit_of_energy_in_MeV.value*1e-3; % in GeV
        e  = phc.electron_volt.value;
        cb = phc.speed_of_light_in_vacuum.value*1e-9; % over 1 billion
    else
