...
%  6. 'BufferSize' 'value' - for set a different value of buffer 
%
%  OUTPUTS
...
BufferSize=1026; % default value for BufferSize
varargin2 = {};
DisplayFlag = 1;
...
for i = length(varargin):-1:1
    ...
    elseif strcmpi(varargin{i},'Struct')
        ...
        varargin2 = [varargin2 varargin{i}];
        varargin(i) = [];
    elseif strcmpi(varargin{i},'BufferSize')      
        BufferSize = str2num(char(varargin(i+1)));
        disp(BufferSize);
        varargin(i) = [];        
        varargin(i) = [];       
    end
...
if FileFlag
    ...
    %make tangodevicelist
    bpmdevtangolist=dev2tangodev('BPMx',elem2dev('BPMx',xpos));
    %set buffersize
    BufferSize=ones(1,length(xpos))*BufferSize;
    DDvect=ones(1,length(xpos));
    DDvect=logical(DDvect);
    writeattribute(strcat(bpmdevtangolist,'/DDBufferSize'),BufferSize,'int32');
    %DDenabled before to get data 
    for i=1:1:size(bpmdevtangolist)
        writeattribute(strcat(bpmdevtangolist(i),'/DDEnabled'),1,'uint8');
    end
    pause(3);
