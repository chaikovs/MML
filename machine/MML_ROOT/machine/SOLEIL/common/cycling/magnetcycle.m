...
%  values
%
% See Also setcyclecurve, getcyclingcurve, plotcyclingcurve, LT1cycling, RINGcycling

%
%  Written by Laurent S. Nadolski
%  Modify by Aurélien Bence in October 2014
...
ConfigFlag = 0; % confirm before modifying cycling ramp
...
if exist('Magnet','var') && ~isempty(Magnet)
    if iscell(Magnet)
        for k = 1:length(Magnet),
            ...
            % Nanoscopium S1/S11
            % if family not defined and S11 family
            if (isempty(flag) && strcmp(Magnet{k}, 'S11'))
                Lattice.S11 = Lattice.S1;
                Lattice.S11.Setpoint.FamilyName = 'S11';
                flag = 1;
            end
            if flag
                ...
                warning('Family %s not configured', Magnet{k});
            end
        end
    end
    Lattice = LatticeN;
...
if isempty(Inom)
    error('Empty nominal value for family %s, skip data');
end
        ...
        case 'StorageRing'
            ...
            switch MagnetType
                case {'BEND'}
                    switch CyclingMode
                        case 'Simple'
                            ...
                            %                                 [Inom 1] ];
                        case 'Full'
                            % valeur initiale differente de zero car zero
                            % impossible
                            curve = [[20 10] %
                                [Imax 60]
                                [0.95*Inom 60]
                ...
                case {'Q1','Q2','Q3','Q4','Q5','Q6','Q7','Q8','Q9','Q10', ...
                        'Q11', 'Q12','QC13', 'S11', 'S12'}
                    % S11 and S12 powersupplies are of quadrupole type 
                    switch CyclingMode
                        case 'Simple'
...
%                                 [Inom 10] ];
                            ...
                            % nanoscopium 1.10*Inom is more than Imax !!
                            curve = [[sign(Imax)*20 10] % current time non zero setpoint value
                                [Imax  30]
                                [0.90*Inom 30]
                                [sign(Imax)*min([abs(Imax), 1.10*abs(Inom)]) 30]
                                [0.90*Inom 30]
                                [sign(Imax)*min([abs(Imax), 1.10*abs(Inom)]) 30]
                                [Inom-sign(Imax)*2 5]
                                [sign(Imax)*min(abs(Inom+sign(Imax)*2), abs(Imax)) 1]
                                [Inom-sign(Imax)*2 1]
                                [sign(Imax)*min(abs(Inom+sign(Imax)*2), abs(Imax)) 1]
                                [Inom-sign(Imax)*2 1]
                                [sign(Imax)*min(abs(Inom+sign(Imax)*2), abs(Imax)) 1]
                    ...
                    switch CyclingMode
                        ...
                        case 'Startup'
                            curve = [[0 10] % current time
                                [Imax 20] % temps de chauffe 10 min
                                [0.90*Inom 60]
                                [sign(Imax)*min([abs(Imax), 1.10*abs(Inom)]) 60]
                                [0.90*Inom 60]
                                [sign(Imax)*min([abs(Imax), 1.10*abs(Inom)]) 60]
                                [Inom 20] ];
                        otherwise
...
if iscell(Family)
    % Build input line
    h = waitbar(0,sprintf('Cycling configuration progression: %2.0f %%',0));
    for k = 1:length(Family)
        cyclingconfiguration(Lattice,Family{k},varargin{:})
        waitbar(k/length(Family), h, sprintf('Cycling configuration progression: %2.0f %%',k/length(Family)*100))
    end
    close(h);
else
    %% INPUT PARSER
    ...
    if ismemberof(CycleFamily,'Cyclage')
        ...
        if strcmpi(Family, 'BEND')
            CycleAO.Inom = CycleAO.Inom(1);
        else
            %check elementList in AO and LatticeFile <Bug> Doesnt work with LT1
            %or LT2 lattice file
%             AOElements=family2elem(CycleFamily,1);
%             LatticeElements=dev2elem(Lattice.(Family).Setpoint.FamilyName, Lattice.(Family).Setpoint.DeviceList);
%             difDeviceList =[setdiff(AOElements,LatticeElements);setdiff(LatticeElements,AOElements)];
%             if ~isempty(difDeviceList)
%                 tangoName=dev2tango(CycleFamily,difDeviceList);
%                 warndlg(tangoName,'Difference status between AO and LatticeFile');
%             end    
        end
        ...
        switch lower(reply)
            case {'y','yes'}
                if strcmpi(Family, 'BEND')
                    dev_name = family2tangodev(CycleAO.FamilyName); 
                else
                    % LSN BUG to be corrected verif --> Need to be Check
                    dev_name = family2tangodev(CycleAO.FamilyName, Lattice.(Family).Setpoint.DeviceList); % Only with specified DeviceList of Lattice File, New component(Status 1) will be ignored
                end
                if size(dev_name,1) == 1 &&  iscell(dev_name)
