   ...
   environment variable. 
	...
	A. Locate the name of the environment variable to set, using the 
	 ...
	 NOTE: When deploying C and C++ standalone applications, it is possible
               to run the shell script file run_kicker_scaling.sh on UNIX and Mac
...
3. Create packages that contain the following files, based on what
   you are deploying: 
	...
	All Platforms:
		* kicker_scaling.ctf (Component Technology File archive,
		  platform-dependent on target machine)
			
	Windows:
		* MCRInstaller.exe (self-extracting MATLAB Component Runtime
		  library archive)  
		* kicker_scaling.exe (application created by MATLAB Compiler)
	UNIX:
		* MCRInstaller.zip (MATLAB Component Runtime library 
	 	  archive - manual unzip required)
		* kicker_scaling (application created by MATLAB Compiler)
		* run_kicker_scaling.sh (shell script that can be run 
	...
	All Platforms:            
		* kicker_scaling.so (shared library, file extension 
		  varies by platform)
		* kicker_scaling.h (library header file)
		* kicker_scaling.ctf (Component Technology File archive,
		  platform-dependent on target machine)
	...
	* kicker_scaling.ctf (Component Technology File archive)	
	* kicker_scaling.xml (documentation files)
	* kicker_scaling.pdb (program database file - if 
	  DEBUG option selected)
	* kicker_scaling.dll (component assembly file)
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  
	...
	* kicker_scaling.ctf (Component Technology File archive)
	* kicker_scaling_1_0.dll (component containing 
	  compiled M-code)
	...
	* kicker_scaling.jar (Java archive containing Java 
	  interface to M-code in kicker_scaling.ctf)
	* kicker_scaling.ctf (Component Technology File archive,
	  platform-dependent on target machine)
