function varargout = synchro_rafale_mode_tests(varargin)
% SYNCHRO_RAFALE_MODE_TESTS M-file for synchro_rafale_mode_tests.fig
%      SYNCHRO_RAFALE_MODE_TESTS, by itself, creates a new SYNCHRO_RAFALE_MODE_TESTS or raises the existing
%      singleton*.
%
%      H = SYNCHRO_RAFALE_MODE_TESTS returns the handle to a new SYNCHRO_RAFALE_MODE_TESTS or the handle to
%      the existing singleton*.
%
%      SYNCHRO_RAFALE_MODE_TESTS('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SYNCHRO_RAFALE_MODE_TESTS.M with the given input arguments.
%
%      SYNCHRO_RAFALE_MODE_TESTS('Property','Value',...) creates a new SYNCHRO_RAFALE_MODE_TESTS or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before synchro_rafale_mode_tests_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to synchro_rafale_mode_tests_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help synchro_rafale_mode_tests
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @synchro_rafale_mode_tests_OpeningFcn, ...
                   'gui_OutputFcn',  @synchro_rafale_mode_tests_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before synchro_rafale_mode_tests is made visible.
function synchro_rafale_mode_tests_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to synchro_rafale_mode_tests (see VARARGIN)


set(handles.edit_etat, 'String','??');
% Choose default command line output for synchro_rafale_mode_tests
...
% delais fixe en pas 
handles.central_dec=int32(1664);
handles.central_nodec=int32(95707);


% Update handles structure
guidata(hObject, handles);

% UIWAIT makes synchro_rafale_mode_tests wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = synchro_rafale_mode_tests_OutputFcn(hObject, eventdata, handles) 
...
% Swap delai 5 et 2
central_inj=handles.central_dec;
central_soft=handles.central_nodec;
...
arg.svalue={'k1.trig','k2.trig','k3.trig','k4.trig'}; arg.lvalue=int32([1 1 1 1])*event0;
...
arg.svalue={'sep-p.trig','sep-a.trig'}; arg.lvalue=int32([1 1])*event0;
...
pause(1)
tango_write_attribute2('ANS/SY/CENTRAL', 'TInjStepDelay', central_inj);
tango_write_attribute2('ANS/SY/CENTRAL', 'TSoftStepDelay',central_soft);
...
% Swap delai 5 et 2
central_inj=handles.central_nodec;
central_soft=handles.central_dec;
tango_write_attribute2('ANS/SY/CENTRAL', 'TInjTimeDelay',290010);
tango_write_attribute2('ANS/SY/CENTRAL', 'TSoftTimeDelay',290020);
