...
% 
% Updated to use ps2pdf now                       07/2009
...
% ps2pdf Command
%-------------------------------------------------------
Str = sprintf('ps2pdf -dNOCACHE %s2.ps %s.pdf',TempName,TempName);
disp(sprintf('[PS2PDF Command] %s',Str));
[hdos,wdos]=system(Str);
if hdos ~=0
    if isunix
        dos(sprintf('rm %s*',TempName));
...
% pdfcrop Command
%-------------------------------------------------------
Str = sprintf('pdfcrop %s.pdf %s.pdf',TempName,FileName);
disp(sprintf('[PDFCROP Command] %s',Str));
[hdos,wdos]=system(Str);
if hdos ~=0
    if isunix
        dos(sprintf('rm %s*',TempName));
