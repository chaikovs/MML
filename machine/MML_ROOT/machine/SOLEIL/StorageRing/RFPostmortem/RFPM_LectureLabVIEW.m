function varargout = RFPM_LectureLabVIEW(varargin)
...
% Last Modified by GUIDE v2.5 31-Mar-2008 18:21:40

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @RFPM_LectureLabVIEW_OpeningFcn, ...
                   'gui_OutputFcn',  @RFPM_LectureLabVIEW_OutputFcn, ...
                   'gui_LayoutFcn',  [], ...
                   'gui_Callback',   []);
...
function RFPM_LectureLabVIEW_OpeningFcn(hObject, eventdata, handles, varargin)
...
function varargout = RFPM_LectureLabVIEW_OutputFcn(hObject, eventdata, handles)
...
%DirectoryName = getfamilydata('Directory', 'RFPostmortem');
%DirectoryName = getfamilydata('Directory', 'RFPostmortem');
cd('/home/production/matlab/matlabML/measdata/SOLEIL/StorageRingdata/Postmortem/RFPostmortem/Labview')
DirStart = pwd;
%DirectoryName = gotodirectory(DirectoryName);
[FileName, DirectoryName] = uigetfile('*.*','Select File (or Cancel)');
%disp (FileName);
...
table=textscan(fid,'%f %f %f %f', 400000);
%table{4}(1:10)

NbrPoint=size(table{1});

%[NbrColumn,NbrPoint] = size (Data.PM1.DataHR1);
...
%table{1}= -(table{1}/5)*10^2;  
%table{2} = ((0.0361 + 0.2542*(table{2}) - 0.0112*(table{2}).^2 + 0.0011*(table{2}).^3).^2/50)*2.74*10^4; 
%table{3} = ((0.0361 + 0.2542*(table{3}) - 0.0112*(table{3}).^2 + 0.0011*(table{3}).^3).^2/50)*8.2*10^2;
%table{4} = (table{4})*10^2; 
...
%axis([valmin valmax mini maxi]);
%set(gca,'XTick',[valmin valmax]);
plot(table{1},'b'); hold on; grid on;
%axis([valmin valmax mini maxi])
plot(table{2},'r');
plot(table{3},'g');
plot(table{4},'k');
