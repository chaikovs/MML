...
%     ListOfXCurrents=horzcat([0:125:250], [275]);       % pas de 125 : 4 points pour essai
    ...
    MaxDelay=10; % délai max pour considérer que les alims ont atteint leur consigne (en secondes)
    TimeToWaitBeforeAcquisition=5; % Temps d'attente avant une acquisition (en secondes)
    ChromaticityMeasurement=0; % Mesure de chromaticité si 1. ATTENTION : PERTURBE LE FAISCEAU!!
    ForceToStop_Ask_ForceToWrite=0; % Si on tente de ré-écrire des courants de correction dans une matrice déjà pleine
...
%Sofb=readattribute('ANS/DG/PUB-SOFB/state');
Sofb=0 
if (Sofb==1)
...
%             end
        ...
        if (ConditionToStoreRefOrbit)  % for helicoidal : &&BXPRESENTCURRENTFORHEL==0)
            if (TESTWITHOUTBEAM==1)
                ...
                REFORBIT=outStructElecBeam.name; % acquisition is made and stored in REFORBIT
            end
            if (debug)
