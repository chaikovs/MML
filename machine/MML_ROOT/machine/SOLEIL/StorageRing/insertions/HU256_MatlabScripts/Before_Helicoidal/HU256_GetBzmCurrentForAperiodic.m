...
%Version Aperiodic
    ...
    if (size(BZPCurrent)==[1, 1])    % BZPCurrent is scalar
        ...
        elseif (B==1)   % BZP Current > max current of BzmMatrix
            fprintf('Error with "HU256_GetBzmCurrentForAperiodic" : The BZP current (%3.3fA) is outside the limits! => Bzm%1.0f will be put to 0A\n', BZPCurrent, BzmNumber)
            return
        else
    ...
    elseif (size(BZPCurrent, 1)==1||size(BZPCurrent, 2)==1) % Vector
        ...
        if (max(BZPCurrent)>BzmMatrix(size(BzmMatrix, 1), 1))  % At least one element of BZPCurrent is outside the BzmMatrix column
            fprintf('Error with "HU256_GetBzmCurrentForAperiodic" : at least one element of BZPCurrent is outside the column n°%1.0f."\n', BzmNumber)
            return
        else
            XVector=BzmMatrix(:, 1);
            YVector=BzmMatrix(:, BzmNumber+1);
        ...
        fprintf('Error with "HU256_GetBzmCurrentForAperiodic" : BZPCurrent should be a vector (1xN or Nx1) but its dimensions are (%fx%f)\n', size(BZPCurrent, 1), size(BZPCurrent, 2));
    end
