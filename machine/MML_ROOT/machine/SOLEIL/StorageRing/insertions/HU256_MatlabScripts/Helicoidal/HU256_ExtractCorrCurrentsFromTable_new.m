...
%                     end
            ...
            if (BXSENSEOFCURRENTFORHEL==1||ConditionOfBothTables==1)  % Extract only from Up Table
                if (strcmp(NameOfUTable, '')==0)
                    ...
                    if (size(Matrix, 2)~=5)
                        fprintf('Problem with HU256_ExtractCorrCurrentsFromTable : The matrix should have 5 columns but it has %f columns\n', size(Matrix, 2))
                        return
                    end
                    if (Matrix==-2) % Pas sur que ça marche!!
                        fprintf('Problem in HU256_ExtractCorrCurrentsFromTable : The matrix couldn''t be extraced from the table\n')
...
%                 end
            ...
            if (BXSENSEOFCURRENTFORHEL==-1||ConditionOfBothTables==1)  % Extract from Down tables
                if (strcmp(NameOfDTable, '')==0)
                    Matrix=HU256_Tab_GetMatrixFromTable(FullNamesOfTableToUseForCorrection, -1, DEBUG);
                    if (size(Matrix, 2)~=9)
                        fprintf('Problem with HU256_ExtractCorrCurrentsFromTable : The matrix should have 9 columns but it has %f columns\n', size(Matrix, 2))
                        return
                    end
                    if (Matrix==-2) % Pas sur que ça marche!!
                        fprintf('Problem in HU256_ExtractCorrCurrentsFromTable : The matrix couldn''t be extraced from the table\n')
...
%                 end    
            ...
            if (ConditionOfBothTables==1)
                if (UResult~=DResult)
                    fprintf('Problem with HU256_ExtractCorrCurrentsFromTable : BX=%f, BZ=%f, SenseOfBZ=%f => Correction currents can be extracted in both tables, but They are not equal\', BXPRESENTCURRENTFORHEL, PRESENTCURRENT, SENSEOFCURRENT)
                    fprintf('Currents from Up Table : CVE=%f, CHE=%f, CVS=%f, CHS=%f\n', UResult(1), UResult(2), UResult(3), UResult(4)')
                    fprintf('Currents from Down Table : CVE=%f, CHE=%f, CVS=%f, CHS=%f\n', DResult(1), DResult(2), DResult(3), DResult(4)')
                    Result=-1;
                    return
                end
            end
