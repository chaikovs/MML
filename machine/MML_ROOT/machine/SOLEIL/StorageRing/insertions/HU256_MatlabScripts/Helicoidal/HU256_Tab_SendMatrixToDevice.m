function Result=HU256_Tab_SendMatrixToDevice(Matrix, HU256Cell, PowerSupply_Or_UpOrDown, Aperiodic)
...
%     DataFolder='/home/operateur/GrpGMI/HU256_CASSIOPEE/MatlabScripts/TempTests';
    ...
    if (isa(PowerSupply_Or_UpOrDown, 'numeric')==1)
        fprintf('Problem in HU256_Tab_SendMatrixToDevice -> PowerSupply_Or_UpOrDown should be a strign\n');
        return
   
        if (strcmp(PowerSupply_Or_UpOrDown, 'bz')==1)
            fprintf('**Debug** bz\n')
            if (Aperiodic==1)
                fprintf(['**Debug** bz\n'])
                UpName='FF_AH_UP.txt';
        ...
        elseif (strcmp(PowerSupply_Or_UpOrDown, 'bx')==1)
        fprintf('**Debug** bx\n')
            if (Aperiodic==1)
                UpName='FF_AV_UP.txt';
            ...
            fprintf('Problem in HU256_Tab_SendMatrixToDevice -> PowerSupply_Or_UpOrDown should be ''bz'' or ''bx''\n');
        end
    elseif (isa(PowerSupply_Or_UpOrDown, 'char')==1)
        if (strcmp(Aperiodic, 'Hel')==1)
            UpName='TableHel';
            DownName='';
        end
    end
    ...
    fprintf(DownFid, '\r\n');
    % For BX : 2nd line = BX2
    if (strcmp(PowerSupply_Or_UpOrDown, 'bx')==1)
        if (Aperiodic==1)
            for column=1:size(Matrix, 1)
                SENSEOFCURRENT=1;
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if (strcmp(PowerSupply_Or_UpOrDown, 'bz')==1&&Aperiodic==1)
        for line=1:8
            for column=1:size(Matrix, 1)
                SENSEOFCURRENT=1;
