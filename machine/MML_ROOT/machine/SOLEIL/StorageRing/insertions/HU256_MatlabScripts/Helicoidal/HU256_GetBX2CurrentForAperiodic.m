...
% If BX1Current is a vector (row or column), returns a vector containing each corresponding value of BX2Current calculated with Cubic Splines
    ...
    BXApMatrix=HU256_GetBX2CurrentMatrixForAperiodic(HU256CELL);
    if (size(BXApMatrix)==[1 1])
        if (BXApMatrix==-1)
            fprintf('Problem in HU256_GetBX2CurrentForAperiodic -> The HU256CELL is incorrect\n')
            return
        end
    end
