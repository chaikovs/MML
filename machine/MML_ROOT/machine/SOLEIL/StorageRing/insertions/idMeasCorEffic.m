...
for i = 1:numMeas
	...
	EIaux = idMeasElecBeamUnd(idName, inclPerturbMeas, fileNameCoreWithCor, 1, dispData);
    
    if (strcmp(EIaux.file, '') == 0)
        if(~isempty(strAllFileNames))
...
%if (length(strAllFileNames) > 0)
    ...
    idSaveStruct(auxListStruct, fileNameCoreList, idName, 1, 0);
end
