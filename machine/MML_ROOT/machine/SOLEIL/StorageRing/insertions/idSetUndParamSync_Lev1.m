...
if useAppleIIDServerLevel1
    ...
    elseif (strcmp(attrName, 'phase') || strcmp(attrName, 'PHASE'))
        attr_name_list = {'encoder5Position','encoder6Position'};
        tango_command_inout2(DServName, 'GotoPhase', [attrValue, attrValue]);
    end
else 
...
for i = 1:numLoopsWaitRes
	...
	if useAppleIIDServerLevel1
        ...
        elseif (strcmp(attrName, 'phase') || strcmp(attrName, 'PHASE'))
            valSet = 0.5*(attr_val_list(1).value + attr_val_list(2).value); %check
        end
    end
