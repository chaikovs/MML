...
for j=1:1:116
    A = fgetl(fidread);
    if (j==60)
        A=['dv0_name:'CellName '/EI/' SSType '-' IDName '/gap']
    end
    if (j==79)
        A=['dv1_name:' CellName '/DG/CALC-SD' SSType '-POSITION-ANGLE/positionX']
    end
    if (j==98)
        A=['dv2_name:'CellName '/DG/CALC-SD' SSType '-POSITION-ANGLE/positionZ']
    end
