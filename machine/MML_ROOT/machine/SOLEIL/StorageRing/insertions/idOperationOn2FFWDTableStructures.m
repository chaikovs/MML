function OutputFFWDTableStructure=idOperationOn2FFWDTableStructures(FFWDTableStructure1, FFWDTableStructure2, PhaseRange, GapRange, Operation)
...
if (CheckResult1==-1)
    fprintf ('Error in ''idOperationOn2FFWDTableStructures'' : FFWDTableStructure1 is incorrect\n');
end
if (CheckResult2==-1)
    fprintf ('Error in ''idOperationOn2FFWDTableStructures'' : FFWDTableStructure2 is incorrect\n');
end
...
%     size(vPhases2, 2)
    ...
    if (sum(vPhases1==vPhases2)==length(vPhases1)&&sum(vGaps1==vGaps2)==length(vGaps1))
        ...
        elseif strcmpi(Operation, 'Average')
            OutputTable=(FFWDTable1+FFWDTable2)./2;
        else
            fprintf ('Error in ''idSumOf2FFWDTableStructures'' : ''Operation'' should be ''+'' or ''Average''\n');
            return
        end
