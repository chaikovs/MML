function varargout = lat_nano_176_234_122BPM_QSD 
...
L0 = 3.540970204203009e+02;% design length [m]
...
Q1   =  quadrupole('Q1' , LQC, -0.1163982E+01, QPassMethod);
Q2   =  quadrupole('Q2' , LQL,  0.1684902E+01, QPassMethod);
Q3   =  quadrupole('Q3' , LQC, -0.6879196E+00, QPassMethod);
Q4   =  quadrupole('Q4' , LQC, -0.1210454E+01, QPassMethod);
Q5   =  quadrupole('Q5' , LQC,  0.1699787E+01, QPassMethod);
Q6   =  quadrupole('Q6' , LQC, -0.1172340E+01, QPassMethod);
Q7   =  quadrupole('Q7' , LQL,  0.2016438E+01, QPassMethod);
Q8   =  quadrupole('Q8' , LQC, -0.1324658E+01, QPassMethod);
Q9   =  quadrupole('Q9' , LQC, -0.1375764E+01, QPassMethod);
Q10  =  quadrupole('Q10', LQC,  0.1734734E+01, QPassMethod);
Q11  =  quadrupole('Q11', LQC, -0.1693858E+01, QPassMethod);
Q12  =  quadrupole('Q12', LQL,  0.1694249E+01, QPassMethod);
QSD  =  quadrupole('QSD', 1e-12,  0.0E+01, QPassMethod);
...
S1  =  sextupole('S1' , Finv,  0.300*5*F, SPassMethod);
S2  =  sextupole('S2' , Finv, -0.715*5*F, SPassMethod);
S3  =  sextupole('S3' , Finv, -0.338*5*F, SPassMethod);
S4  =  sextupole('S4' , Finv,  0.697*5*F, SPassMethod);
S5  =  sextupole('S5' , Finv, -0.672*5*F, SPassMethod);
...
S9  =  sextupole('S9' , Finv, -0.791*5*F, SPassMethod);
S10 =  sextupole('S10', Finv,  0.468*5*F, SPassMethod);
S11 =  sextupole('S11', Finv,  0.300*5*F, SPassMethod);
...
SDL01d = [QSD SD1a   PtINJ   SD1b   K3  SD1c1 FCOR  SD1c2  KEMH SD1c3u VSCRAP SD1c3d K4 SD1d];
...
SDL05  = [HU640upstream QSD HU640downstream]; % DESIRS HU640
SDL09  = [SD1e QSD SD1e];
...
SDL13 = [SDL13u Q12 QSD SDL13d];
%SDL13  = [SD1e SD1e]; % NANOSCOPIUM U20 + TOMOGRAPHY U18 CRYO

% 12 medium straigt sections (7 m, available part for IDs 5.46 m)
SDM02 = [SD13a QSD CAV SD13a]; % CRYOMODULE #2
SDM03 = [SD13a QSD SD13a]; % CRYOMUDULE #1 not put in the model for simplicity
SDM04 = [SD13a QSD SD13a]; % PLEIADES HU256 + HU80
SDM06 = [SD13a QSD SD13a]; % PUMA future Wiggler
%SDM06 = [SDWSV50 SWSV50 SDWSV50]; % PUMA future Wiggler
SDM07 = [SD13a QSD SD13a]; % DEIMOS HU52+EMPHU65
SDM08 = [SD13a QSD SD13a]; % TEMPO HU80+HU44
SDM10 = [SD13a QSD SD13a]; % HERMES HU64+HU42
SDM11 = [SDPX2a PX2C CHIPX2D1 SDPX2b PX2C QSD CHIPX2D2 SDPX2b CHIPX2D3 PX2C SDPX2c]; % PX2 U24
SDM12 = [SD13a QSD SD13a]; % ANTARES HU256 + HU60
SDM14 = [SD13a QSD SD13a]; % SEXTANTS (ex microFocus) HU44 + HU80
SDM15 = [SD13a QSD SD13a]; % CASSIOPEE HU256 + HU80
SDM16 = [SD13a QSD SD13a]; % LUCIA HU52

% 8 short straigt sections (3.6 m, available part for IDs 2.8 m)
SDC02 = [SDAC1 QSD SDAC1];% LIGNE ALPHA
SDC03 = [SDAC1 QSD SDAC1];% PSICHE WSV50
SDC06 = [SDAC1 QSD SDAC1];% CRISTAL U20
SDC07 = [SDAC1 QSD SDAC1];% GALAXIES U20
SDC10 = [SDAC1 QSD SDAC1];% PX1 U20
SDC11 = [SDAC1 QSD SDAC1];% SWING U20
SDC14 = [SDAC1 QSD SDAC1];% SIXS U20
SDC15 = [SDAC1 QSD SDAC1];% SIRIUS HU34
