...
%
% Partie Anneau modifiï¿½e par P. Brunelle et A. Nadji le 31/03/06
%
...
% 21 octobre 2008 - P. Brunelle - Qpoles anneau - introduction des coefficents déduits de
% l'étalonnage en courant utilisant les vraies valeurs des courants. Les anciens
% coefficients sont commentés.

% 7 mai 2009 - P. Brunelle - Spoles anneau - introduction des coefficents déduits de
% l'étalonnage en courant utilisant les vraies valeurs des courants + répartition par intervalle de courant.

% 12 juin 2009 - P. Brunelle - Qpoles anneau - répartition par intervalle de courant.
...
%                 a0 =  0.0;
    ...
    case 'StorageRing'
        ...
        coeffQ =    0e-3   ;  %0e-3  ;  %  0e-3     %  0e-3 ;   %  0e-3      ;  %  8e-3  ;   % appliqué sur le premier faisceau
...
%                 case {} 
%                 % CAS DU QUADRUPOLE COURT DONT LE COURANT EST COMPRIS ENTRE 100 et 150A
%                 % POLARITE - pour le courant MAIS POLARITE + pour le gradient
%                 Leff=LtotQC;
%                 a7=  0.0;
%                 a6=  0.0;
%                 a5=  0.0;
%                 a4=  0.0;
%                 a3=  0.0;
%                 a2=  -1.72242E-6*(1.003)*bob;
%                 a1=  2.78608E-2*(-1)*(1.003)*bob;
%                 a0=  4.86245E-3*(1.003)*bob;        
%                 A = [a7 a6 a5 a4 a3 a2 a1 a0];                
%                 MagnetType = 'quad';
                ...
                a4=  0.0;
                a3=  0.0;
                a2=  -9.77342E-6*(1.003)*bob;
                a1=  3.03524E-2*(-1)*(1.003)*bob;
                a0=  -1.88248E-1*(1.003)*bob;        
                A = [a7 a6 a5 a4 a3 a2 a1 a0];                
                MagnetType = 'quad'; 
...
%                 case {''} 
%                 % CAS DU QUADRUPOLE LONG DONT LE COURANT EST COMPRIS ENTRE 80 et 135A
%                 % POLARITE + 
%                 Leff=LtotQL;
%                 a7=  0.0;
%                 a6=  0.0;
...
%                 a4=  0.0;
%                 a3=  0.0;
%                 a2=  -2.55217E-6*(1.0155)*bob;
%                 a1=  4.50695E-2*(1.0155)*bob;
%                 a0=  6.10246E-3*(1.0155)*bob;        
%                 A = [a7 a6 a5 a4 a3 a2 a1 a0];                
%                 MagnetType = 'quad';
...
%             case{}
%                 % CAS DU SEXTUPOLE DONT LE COURANT EST COMPRIS ENTRE 0 et 60A
%                 % POLARITE (courant) + mais H - car alimentation sextupole "retournée"
%                 Leff=LtotSX;
%                 a7=  0.0;
%                 a6=  0.0;
%                 a5=  0.0;
...
%                 % POLARITE +
                ...
                MagnetType = 'SEXT';
            case {'S4'}
                 % CAS DU SEXTUPOLE DONT LE COURANT EST COMPRIS ENTRE 330 et 350A
                 % polarité positive 
                Leff=LtotSX;
                a7=  0.0;
                a6=  0.0;
...
%          case {}       
%  %                 % CAS DU SEXTUPOLE DONT LE COURANT EST COMPRIS ENTRE 310 et 330A
%  %                 % polarité positive 
%                 Leff=LtotSX;
%                 a7=  0.0;
%                 a6=  0.0;
...
%%                
            ...
            case 'QT'    % 160 mm dans sextupole
                % Etalonnage: moyenne sur les 32 sextupï¿½les incluant un QT.
                % Efficacite = 3 G.m/A @ R=32mm; soit 93.83 G/A
                % Le signe du courant est donnï¿½ par le DeviceServer (Tango)
            ...
            case {'HCOR'}    % 16 cm horizontal corrector
                % Etalonnage: moyenne sur les 56 sextupï¿½les incluant un CORH.
                % Efficacitï¿½ = 8.143 G.m/A 
                % Le signe du courant est donnï¿½ par le DeviceServer (Tango)
                % Find the currAO.(ifam).Monitor.HW2PhysicsParams{1}(1,:) = magnetcoefficients(AO.(ifam).FamilyName );
                Leff = 0.16;
                a7= 0.0;
            ...
            case {'VCOR'}    % 16 cm vertical corrector
                % Etalonnage: moyenne sur les 56 sextupï¿½les incluant un CORV.
                % Efficacitï¿½ = 4.642 G.m/A 
                % Le signe du courant est donnï¿½ par le DeviceServer (Tango)
                % Find the currAO.(ifam).Monitor.HW2PhysicsParams{1}(1,:) = magnetcoefficients(AO.(ifam).FamilyName );
                Leff = 0.16;
                a7= 0.0;
            ...
            case {'K_INJ'}
             ...
             case {'K_INJ1'}
                ...
                SBDL = 75.230e-3 ; % somme de Bdl mesurÃ©e
                MagnetType = 'K_INJ1';
             ...
             case {'K_INJ2'}
                ...
                Leff = .6;
                vmax = 7500;% tension de mesure
                SBDL = 74.800e-3 ; % somme de Bdl mesurÃ©e
                MagnetType = 'K_INJ2';  
                A = [0 SBDL/vmax 0]*Leff; 
            ...
            case {'SEP_P'}
                ...
                SBDL = 263e-3; % somme de Bdl mesurÃ©e
                MagnetType = 'SEP_P';
                A = [0 SBDL/vmax 0]*Leff; 
             ...
             case {'SEP_A'}
                ...
                vmax = 111;
                MagnetType = 'SEP_A';
                SBDL = 1147.8e-3 ; % Somme de Bdl mesurÃ©e Ã  111 V
                A = [0 SBDL/vmax 0]*Leff; 
    ...
    case 'LT2'
        ...
        switch upper(deblank(MagnetCoreType))
            ...
            case 'BEND'    
                % les coefficients et longueur magnÃ©tique sont recopiÃ©s de l'anneau
                Leff=1.052433;
                a7= 0.0;
                a6=-0.0;
                a5= 0.0;
            ...
            case {'QP'}   % 400 mm quadrupole
                ...
                % le rÃ©manent est + fort que pour les quad Booster car les
