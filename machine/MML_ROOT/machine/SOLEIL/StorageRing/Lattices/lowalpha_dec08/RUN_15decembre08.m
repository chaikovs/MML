...
IQ10 = [  -82.9650  148.9879 -114.2645 -119.1887  201.2604 -175.5612  121.0107 -82.0893 -154.9695  217.9303 ];

IS10 = [  24.4811  -99.0941 -229.7569  266.4587 -133.3639  144.5484 -116.8993  132.9682 -218.2526  260.3938];

% alphaby20

IQ20 = [  -84.4498  149.0530 -111.5045 -118.4300  201.4291 -172.7899  107.6342  -101.5160 -153.4449  218.3537  ];

IS20 = [  24.4811  -99.0941 -229.7569  266.4587 -133.3639  144.5484 -116.8993  132.9682 -218.2526  260.3938];

% alphaby30

IQ30 = [ -83.8828  149.2383 -113.4019 -118.4225  201.5454 -170.4938  101.9189  -109.0886 -153.6022  218.5726 ];

IS30 = [  28.5769  -99.0941 -223.7643  266.4587 -127.4202  140.1915 -116.8993  126.6359 -217.7679  260.3938 ];

% alphaby40

IQ40 = [  -84.5960  149.5563 -113.2614 -118.1814  201.5609 -169.6577   98.8093  -113.1637 -153.2026  218.6095 ];

IS40 = [ 31.7265  -99.0941 -223.9622  266.4587 -129.2957  141.8893 -116.8993  125.8615 -214.4666  260.3938 ];

% alphaby50

IQ50 = [ -84.7413  149.6209 -113.2523 -118.0724  201.5714 -169.1231   97.0450  -115.4381 -153.0419  218.6456 ];

IS50 = [ 29.3099  -99.0941 -227.1668  266.4587 -133.0502  142.4918 -116.8993  127.7213 -212.5219  260.3938 ];

% alphaby60

IQ60 = [ -84.6258  149.5861 -113.3963 -117.9293  201.5579 -168.7408   95.7023  -117.1696 -152.8836  218.6629 ];

IS60 = [ 27.7293  -99.0941 -227.7373  266.4587 -133.5530  142.9736 -116.8993  129.2884 -212.7773  260.3938 ];

QuadMat  = [IQ10; IQ20; IQ30; IQ40; IQ50; IQ60];
SextuMat = [IS10; IS20; IS30; IS40; IS50; IS60];
...
iTarget=6;
Quad_DK = Quad_dRoR(iTarget,:);

setsp('Q1' ,(k* Quad_DK(1)  + 1)*IQ10(1), Mode, Units);
setsp('Q2' ,(k* Quad_DK(2)  + 1)*IQ10(2), Mode, Units);
setsp('Q3' ,(k* Quad_DK(3)  + 1)*IQ10(3), Mode, Units);
setsp('Q4' ,(k* Quad_DK(4)  + 1)*IQ10(4), Mode, Units);
setsp('Q5' ,(k* Quad_DK(5)  + 1)*IQ10(5), Mode, Units);
setsp('Q6' ,(k* Quad_DK(6)  + 1)*IQ10(6), Mode, Units);
setsp('Q7' ,(k* Quad_DK(7)  + 1)*IQ10(7), Mode, Units);
setsp('Q8' ,(k* Quad_DK(8)  + 1)*IQ10(8), Mode, Units);
setsp('Q9' ,(k* Quad_DK(9)  + 1)*IQ10(9), Mode, Units);
setsp('Q10',(k* Quad_DK(10) + 1)*IQ10(10), Mode, Units);
...
%%
Sextu_DK = Sextu_dRoR(iTarget,:);
setsp('S1' ,(k* Sextu_DK(1)  + 1)*IS10(1), Mode, Units);
setsp('S2' ,(k* Sextu_DK(2)  + 1)*IS10(2), Mode, Units);
setsp('S3' ,(k* Sextu_DK(3)  + 1)*IS10(3), Mode, Units);
setsp('S4' ,(k* Sextu_DK(4)  + 1)*IS10(4), Mode, Units);
setsp('S5' ,(k* Sextu_DK(5)  + 1)*IS10(5), Mode, Units);
setsp('S6' ,(k* Sextu_DK(6)  + 1)*IS10(6), Mode, Units);
setsp('S7' ,(k* Sextu_DK(7)  + 1)*IS10(7), Mode, Units);
setsp('S8' ,(k* Sextu_DK(8)  + 1)*IS10(8), Mode, Units);
setsp('S9' ,(k* Sextu_DK(9)  + 1)*IS10(9), Mode, Units);
setsp('S10',(k* Sextu_DK(10) + 1)*IS10(10), Mode, Units);
