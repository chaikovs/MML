...
CAV = rfcavity('RF' , 0 , 3.2e+6 , HarmNumber*C0/L0, ...
...
S9  =  sextupole('S9' , Finv, -3.72413*F, 'StrMPoleSymplectic4Pass');
S10 =  sextupole('S10', Finv,  2.22559*F, 'StrMPoleSymplectic4Pass');
...
if ismac
    k = 0;
    kmax = 10; % exception number
    while k==0 | k < kmax | strcmp(exception.message, 'Library or function could not be loaded')
        try
            atsummary;
            break; % if no exception break from the loop
        catch exception
            k = k + 1
        end
    end
