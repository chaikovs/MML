function varargout = lat_pseudo_nanoscopium_juin2011_122BPM_ThickSextu 
...
L0 = 3.540970192202998e+02;% design length [m]
...
%% SCRAPER
HSCRAP =  aperture('HSCRAP', [-0.035, 0.035, -0.05, 0.05]*1e3,'AperturePass');
VSCRAP =  marker('VSCRAP', 'IdentityPass');
mSDC15 = aperture('mSDC15', [-0.025, 0.035, -0.05, 0.05]*1e3,'AperturePass');

%INJ = aperture('INJ',[-0.035 0.035 -0.0125 0.0125]*100,'AperturePass');

%% Elements in Injection section
PtINJ = marker('PtINJ', 'IdentityPass');
K1 = corrector('K1',0.0,[0 0],'IdentityPass');
K2 = corrector('K2',0.0,[0 0],'IdentityPass');
K3 = corrector('K3',0.0,[0 0],'IdentityPass');
K4 = corrector('K4',0.0,[0 0],'IdentityPass');
...
Q1   =  quadrupole('Q1' , LQC, -1.26474,  QPassMethod);
Q2   =  quadrupole('Q2' , LQL,  1.709483, QPassMethod);
Q3   =  quadrupole('Q3' , LQC, -0.6532966,QPassMethod);
Q4   =  quadrupole('Q4' , LQC, -1.191092, QPassMethod);
Q5   =  quadrupole('Q5' , LQC,  1.70555,  QPassMethod);
Q6   =  quadrupole('Q6' , LQC, -1.232844, QPassMethod);
Q7   =  quadrupole('Q7' , LQL,  2.05284,  QPassMethod);
Q8   =  quadrupole('Q8' , LQC, -1.396382, QPassMethod);
Q9   =  quadrupole('Q9' , LQC, -1.334399, QPassMethod);
Q10  =  quadrupole('Q10', LQC,  1.727884, QPassMethod);
Q11  =  quadrupole('Q11', LQC, -1E-10 , QPassMethod);
Q12  =  quadrupole('Q12', LQL,  1E-10, QPassMethod);
...
S1  =  sextupole('S1' , Lsextu,  0.343838*5*Finv, SPassMethod);
S2  =  sextupole('S2' , Lsextu, -0.6283534*5*Finv, SPassMethod);
S3  =  sextupole('S3' , Lsextu, -0.4724375*5*Finv, SPassMethod);
S4  =  sextupole('S4' , Lsextu,  0.8128982*5*Finv, SPassMethod);
S5  =  sextupole('S5' , Lsextu, -0.755816*5*Finv,  SPassMethod);
S6  =  sextupole('S6' , Lsextu,  0.6088501*5*Finv, SPassMethod);
S7  =  sextupole('S7' , Lsextu, -1.030959*5*Finv,  SPassMethod);
S8  =  sextupole('S8' , Lsextu,  0.8083523*5*Finv, SPassMethod);
S9  =  sextupole('S9' , Lsextu, -0.7188769*5*Finv, SPassMethod);
S10 =  sextupole('S10', Lsextu,  0.4461458*5*Finv, SPassMethod);
S11 =  sextupole('S11', Lsextu,  0.343838*5*Finv,  SPassMethod);
S12 =  sextupole('S12', Lsextu,  1.000E-10*Finv, SPassMethod); % to be drawn by drawlattice
...
BEND1  =  rbend2('BEND', 1.05243/2, theta/2, thetae, thetas*0, K,fullgap,'BndMPoleSymplectic4Pass');
BEND2  =  rbend2('BEND', 1.05243/2, theta/2, thetae*0, thetas, K,fullgap,'BndMPoleSymplectic4Pass');
%BEND = [BEND1 BEND2];
BEND1  =  rbend2('BEND', 1.05243/6, theta/6, thetae, thetas*0, K,fullgap,'BndMPoleSymplectic4Pass');
BEND2  =  rbend2('BEND', 1.05243/3*2, theta/3*2, thetae*0, thetas*0, K,fullgap,'BndMPoleSymplectic4Pass');
BEND3  =  rbend2('BEND', 1.05243/6, theta/6, thetae*0, thetas, K,fullgap,'BndMPoleSymplectic4Pass');
%BEND = [BEND1 BEND2];
...
SD2 = drift('SD2',  0.369900-Lsextu, 'DriftPass');
SD3 = drift('SD3',   0.181900-Lsextu, 'DriftPass');
SD5 = drift('SD5',  0.179900-Lsextu, 'DriftPass');
SD6 = drift('SD6',  0.79000-Lsextu, 'DriftPass');
SD7 = drift('SD7',  0.419900, 'DriftPass');
SD8 = drift('SD8',  0.179900-Lsextu, 'DriftPass');
SD12= drift('SD12', 0.44990-Lsextu, 'DriftPass');
SD12x= drift('SD12', 0.36566-LQC/2, 'DriftPass'); % upstream H-scraper external % 
SD12y= drift('SD12', 0.44990-(0.36566-LQC/2)-Lsextu, 'DriftPass'); % downstream H-scraper
SD12u= drift('SD12', 0.2, 'DriftPass'); % upstream V V-scraper % TO BE UPDATED
SD12d= drift('SD12', 0.44990-0.2-Lsextu, 'DriftPass'); % downstream H-scraper
...
SD41 = drift('SD41',  0.2521-Lsextu, 'DriftPass');
SD42 = drift('SD42',  0.205-Lsextu, 'DriftPass');
SD92 = drift('SD92',  0.204300, 'DriftPass');
SD93 = drift('SD93',  0.251300	, 'DriftPass');
SD43 = drift('SD43', 0.2051-Lsextu	, 'DriftPass');
SD141 = drift('SD141', 0.431900, 'DriftPass');
SDB1 = drift('SDB1', 0.29100, 'DriftPass');
SDB2 = drift('SDB2', 0.16680000, 'DriftPass');
SDB3 = drift('SDB3', 0.252-Lsextu,  'DriftPass');
SDB4 = drift('SDB4', 0.2776-Lsextu ,'DriftPass');
SDB5 = drift('SDB5', 0.205-Lsextu  ,'DriftPass');
SDB6 = drift('SDB6', 0.119800 ,'DriftPass');
SDB7 = drift('SDB7', 0.166900 , 'DriftPass');
SDB8 = drift('SDB8', 0.252-Lsextu , 'DriftPass');
SDB9 = drift('SDB9', 0.119800 ,'DriftPass');
SDB10= drift('SDB10',0.166900 , 'DriftPass');
SDB11= drift('SDB11',0.2519-Lsextu, 'DriftPass');
SDB12= drift('SDB12',0.2049-Lsextu, 'DriftPass');
SDB13= drift('SDB13',0.119800, 'DriftPass');
SDB14= drift('SDB14',0.1668000, 'DriftPass');
SDB15= drift('SDB15',0.252-Lsextu , 'DriftPass');
SDB17= drift('SDB17',0.205-Lsextu , 'DriftPass');
...
SDC4 = drift('SDC4' , 0.3358-Lsextu,  'DriftPass');
SDC5 = drift('SDC5' , 0.0846,  'DriftPass');
SDC6 = drift('SDC6' , 0.079,   'DriftPass');
SDC7 = drift('SDC7' , 0.342-Lsextu ,  'DriftPass');
...
DRFT12= drift('DRFT12',0.3358-Lsextu , 'DriftPass');
DRFT13= drift('DRFT13',0.0846 , 'DriftPass');
DRFT14= drift('DRFT14',0.0788 , 'DriftPass');
DRFT15= drift('DRFT15',0.3422-Lsextu , 'DriftPass');
...
SDNANO7 = drift('SDNANO7',  0.4100-FAMLIST{Q11}.ElemData.Length/2-Lsextu, 'DriftPass'); % Q11.1 - S12
SDNANO8 = drift('SDNANO8',  0.4800-FAMLIST{Q12}.ElemData.Length/2-Lsextu, 'DriftPass'); % S12 - Q11
...
SDC15 = [mSDC15 SDAC1  SDAC1];% SIRIUS HU34
...
SUP1  = [...
    ...
    BPM  SDB5  SX3    SD8   Q4      SD7     PXBPM BEND ...
    SD7     Q6      SD5    SX5     SD41    ...
...
SUP4  = [  ...
    ...
    SDB7  BPM    SD42    SX5     SD5    Q6      SD7    ...
    BEND   SD7   Q4      SD8     SX3  SDB3 ...
    BPM     SD9a   Q5   SD12x HSCRAP  SD12y   ...
    SX4   SDB4  BPM     SD10a    Q5     SD93 ...
    BPM  SD42  SX3     SD8    Q4     SD7   ...
    BEND    SD6     SX2      SD5     Q3     SD141 ...
    BPM  SDB18  Q2     SD3   SX1     SD2     Q1 ...
