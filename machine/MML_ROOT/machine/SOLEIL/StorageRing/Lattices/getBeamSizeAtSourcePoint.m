...
%% Run the lattice with markers
% carefull !  update if new lattice
%setdevelopmentmode(45) 
%setoperationalmode(60)
%% List of beamlines
...
DirStart = pwd;
cd(getfamilydata('Directory', 'Lattice'))
...
fprintf(fid, ['BeamLineName     Long_Position(m)    RMS_H_Size(microm)    RMS_V_Size(microm)      betaX(m)    betaZ(m)   etax(m)   etaxp(-)   etaz(m)   etazp(m)   alphax(-)   alphaz(-) \n']); 
...
for j= 1:length(REFPTS)
    ...
    fprintf(fid, [BeamLineStructure(j).name '     %10.4f      %6.1f      %6.1f %10.4f      %10.4f     %10.4f  %10.4f      %10.4f     %10.4f  %10.4f      %10.4f  \n'],...
        BeamLineStructure(j).position,BeamLineStructure(j).x,BeamLineStructure(j).y,...
