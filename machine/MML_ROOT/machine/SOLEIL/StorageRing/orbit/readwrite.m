...
%             FileName = getfamilydata('OpsData', 'BPMGoldenFile');
            ...
            [DirectoryName, ErrorFlag] = gotodirectory(fullfille(DirectoryName));
            if strcmp(tmp,'NO')
        ...
        switch refplane % Select plane to save
            case 'X' % Horizontal plane
                % Old Field BPMxData           = getx('struct');
                Data1           = getx('struct');
                % Modified creator field
                Data1.CreatedBy ='solorbit-readwrite';
...
%                     save GoldenPhysData PhysData
                    FileName = appendtimestamp(['Golden',getfamilydata('Default', 'BPMArchiveFile'),'x'], clock);
                    save(FileName, 'Data1');
                    setphysdata(BPMxFamily,'Golden',Data1);
            ...
            case 'Z' % Vertical plane
                % Old Field BPMzData           = getz('struct');
                Data2           = getz('struct');
                Data2.CreatedBy = 'solorbit-readwrite';
...
%                     PhysData.BPMz.Golden = BPMzData;                    
                      FileName = appendtimestamp(['Golden',getfamilydata('Default', 'BPMArchiveFile'),'z'], clock);
                      save(FileName, 'Data2');
                      setphysdata(BPMzFamily,'Golden',Data2);
...
%                 BPMzData.iFit      = BPM(2).ifit;
                if GoldenFlag
                    FileName = appendtimestamp(['Golden',getfamilydata('Default', 'BPMArchiveFile')], clock);
                    save(FileName,'Data1', 'Data2');
                    setphysdata({BPMxFamily, BPMzFamily},{'Golden', 'Golden'}, {Data1, Data2});
%                     physdatafile=getfamilydata('OpsData', 'PhysDataFile');
%                     datastruct=load(physdatafile);
