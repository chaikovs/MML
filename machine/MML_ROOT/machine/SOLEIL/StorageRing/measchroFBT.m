function [Chromaticity, FileName] = measchroFBT(varargin)
...
if strcmpi(RFUnitsString, 'MHz')
    if abs(max(DeltaRF)-min(DeltaRF)) > .0001;  % .001 MHz
...
elseif strcmpi(RFUnitsString, 'kHz')
    if abs(max(DeltaRF)-min(DeltaRF)) > 1;  % kHz
...
elseif strcmpi(RFUnitsString, 'Hz')
    if abs(max(DeltaRF)-min(DeltaRF)) > 1000;  % Hz
...
if isfamily('TUNEFBT')
    c.Monitor = family2datastruct('TUNEFBT','Monitor',[1 1;1 2]);
else
    c.Monitor = gettuneFBT('Struct', 'Model');  % Just to fill the structure
    c.Monitor.Data = NaN * c.Monitor.Data;
end
c.Monitor = gettuneFBT('Struct', ModeFlag);     % Should WaitFlag before to be sure to get a good measurement ?????
c.Actuator = RFsp;
...
if strcmpi(ModeFlag,'Model')
    ...
    if strcmpi(UnitsFlag,'Physics')
        TuneUnitsString = getfamilydata('TUNEFBT','Monitor','PhysicsUnits');
