...
%rep='/home/production/matlab/matlabML/measdata/Ringdata/BPM'
...
%     BpmDevList(i,:);
%     InitValBpm(i)=getam(BpmFam,BpmDevList(i,:)); % MAT c'est long !! A modifier: pas très performant
%     BpmSigma0(i)=1;
% end

InitValBpm(1:length(BpmDevList))=getam(BpmFam,BpmDevList);
BpmSigma0(1:120)=ones(1,120);
...
%     InitValCor(i)=getsp(CorFam,CorDevList(i,:)); % MAT c'est long ! A modifier: pas très performant
% end
InitValCor(1:length(CorDevList))=getsp(CorFam,CorDevList); % MAT c'est long ! A modifier: pas très performant
...
%% $$  Début du BBA   $$ %%
    if strcmpi(Method,'MEC')
        ...
        for v=1:nbDeltaQp
            ...
            %pause(3); % modif 9 septembre de 6 à 3 s
            pause(6); % modif 23 juin 08 de 3 à 6 s
            ...
            for u=1:nbDeltaCor
                ...
                if u==1
                    %pause(4); % défini par l'expérience - modification 9 septembre passage de 9s à 4s
                    pause(9); % défini par l'expérience - modification 23 juin 08 passage de 9s à 4s
                end
                ...
                %%tic
                BpmVal(1:120,v,u)=getam(BpmFam,BpmDevList);
                %for w=1:length(BpmDevList)
                    %BpmVal(w,v,u)=getam(BpmFam,BpmDevList(w,:)); %BpmVal(Bpm,Quad,Cor)
                %end
                %%toc
                %%%%%%%%%%%%%%%%%%%%% test si l'orbite dépasse l'interlock en V  MAT 9 septembre 07
                A = BpmVal(1:120,v,u) ;
                I = find(abs(A)>0.6); % critère du test = 0.7 mm en absolu
                if ~isempty(I)&Plane==2
                    disp(strcat('Quadrupole :',QuadFam{1},' * ',num2str(QuadDev))); % nom du quad et valeur du deltaQ
                    fprintf('delta I sur ce quad : %6.2f  A\n',DeltaQp);
                    disp( strcat('Correcteur :',CorFam,' * ',num2str(CorDevListRes))); % nom du correcteur et valeur du deltacor
                    fprintf('delta I sur ce corr : %6.2f  A\n',DeltaCor);
                    fprintf('valeurs max de l''orbite %6.2f %6.2f mm\n',max(A(I)),min(A(I)));
                    disp('****************************');
                end
                %%%%%%%%%%%%%%%%%%%%%    
