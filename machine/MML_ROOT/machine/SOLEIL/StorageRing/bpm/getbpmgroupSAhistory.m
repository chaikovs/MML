function [AM, tout, DataTime, ErrorFlag] = getbpmgroupSAhistory(varargin)
...
R = tango_group_read_attributes(GroupID, {'XPosSAHistory', 'ZPosSAHistory'}, 0);
...
% construct data
AM.Data.X=zeros(length(R.dev_replies),R.dev_replies(1).attr_values(1).n);
AM.Data.Z=zeros(length(R.dev_replies),R.dev_replies(1).attr_values(1).n);
for k = 1:length(R.dev_replies),
    AM.Data.X(k,:) = R.dev_replies(k).attr_values(1).value;
    AM.Data.Z(k,:) = R.dev_replies(k).attr_values(2).value;
end
