function [AM, tout, DataTime, ErrorFlag] = getzwithXBPMaverage(varargin)
% getzwithXBPM
%
%  INPUTS
...
%
% See Also getphotonbpmaverage, getz, getvbpmaverage
...
if ~isempty(XBPMDeviceList),
    ...
    elseif strcmpi(Mode,'Online')% online
        zXBPM = getphotonbpmaverage('BPMz', 'Monitor', varargin2{:}); % Hardware units mandatory
    end
end
...
if ~isempty(BPMDeviceList),
    if strcmpi(Mode,'Online')
        ...
        AM(iBPM) = getvbpmaverage('BPMz', 'Monitor', BPMDeviceList, 'Hardware');
    else
