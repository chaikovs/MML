function THERING=modelsetmultipoleerrors(THERING, varargin)
% MODELSETMULTIPOLEERRORS - add mutipole to a lattice
...
for i = length(varargin):-1:1,
    if strcmpi(varargin{i},'BEND')
        bendFlag = 1;
    elseif strcmpi(varargin{i},'QUAD')
        quadFlag = 1;
    elseif strcmpi(varargin{i},'SEXTU')
        sextuFlag = 1;
    elseif strcmpi(varargin{i}, 'HCOR')
        hcorFlag = 1;
    elseif strcmpi(varargin{i}, 'VCOR')
        vcorFlag = 1;
    elseif strcmpi(varargin{i}, 'QT')
        qtFlag = 1;
    end
end
...
if bendFlag
    ...
    for i = 1:length(BIndex),      
        THERING{BIndex(i)}.MaxOrder= MaxOrder-1;        
        THERING{BIndex(i)}.PolynomA(MaxOrder) = 0.0;
        THERING{BIndex(i)}.PolynomB(MaxOrder) = 0.0;
        for j = 1:length(Berr)            
            k     = Berr{j}.order;
            xi    = Berr{j}.xi;
            dBoBn = Berr{j}.dBoBn;            
            if  k==2
                THERING{BIndex(i)}.PolynomB(k) = THERING{BIndex(i)}.PolynomB(k)+rhoi*dBoBn*xi;                
            else
                THERING{BIndex(i)}.PolynomB(k) = rhoi*dBoBn*xi;
            end
        end
    end    
end
...
if quadFlag
    ...
    for i = 1:length(QLIndex),
        THERING{QLIndex(i)}.MaxOrder= MaxOrder-1;
        THERING{QLIndex(i)}.PolynomA(MaxOrder) = 0.0;
        THERING{QLIndex(i)}.PolynomB(MaxOrder) = 0.0;
        for j = 1:length(QLerr)
            k     = QLerr{j}.order;
            xi    = QLerr{j}.xi;
            dBoBn = QLerr{j}.dBoBn;
            THERING{QLIndex(i)}.PolynomB(k) = THERING{QLIndex(i)}.PolynomB(2)*dBoBn*xi;
        end
    end
    ...
    for i = 1:lenQSIndex,
        THERING{QSIndex(i)}.MaxOrder = MaxOrder-1;
        THERING{QSIndex(i)}.PolynomA(MaxOrder) = 0.0;
        THERING{QSIndex(i)}.PolynomB(MaxOrder) = 0.0;
        for j = 1:lenQSerr
            k     = QSerr{j}.order;
            xi    = QSerr{j}.xi;
            dBoBn = QSerr{j}.dBoBn;    
            THERING{QSIndex(i)}.PolynomB(k) = THERING{QSIndex(i)}.PolynomB(2)*dBoBn*xi;
        end
    end
...
if qtFlag
    ...
    fid     = fopen('/Users/nadolski/codes/tracy/maille/soleil/corqt.txt','r');
    ...
    for i = 1:length(SKQIndex),
        THERING{SKQIndex(i)}.MaxOrder= MaxOrder-1;
        THERING{SKQIndex(i)}.PolynomA(MaxOrder) = 0.0;
        THERING{SKQIndex(i)}.PolynomB(MaxOrder) = 0.0;
        THERING{SKQIndex(i)}.PolynomB(k) = dBoB4*skqcorr(i)*conv/brho*x02i;
    end
...
if sextuFlag
    ...
    for i = 1:length(SIndex),
        THERING{SIndex(i)}.MaxOrder= MaxOrder-1;
        THERING{SIndex(i)}.PolynomA(MaxOrder) = 0.0;
        THERING{SIndex(i)}.PolynomB(MaxOrder) = 0.0;
        for j = 1:length(Serr),
            k     = Serr{j}.order;
            xi    = Serr{j}.xi;
            dBoBn = Serr{j}.dBoBn;
            THERING{SIndex(i)}.PolynomB(k) = THERING{SIndex(i)}.PolynomB(3)*dBoBn*xi;
        end
    end
end
...
if hcorFlag
    ...
    brho   = getbrho;
    conv   = hw2physics('HCOR','Setpoint',1,HCIndex(1)); % conversion des A en T.m
    
    fid   = fopen('/Users/nadolski/codes/tracy/maille/soleil/corh57.txt','r');
    hcorr = fscanf(fid,'%f');
    ...
    for i = 1:length(HCIndex),
        THERING{HCIndex(i)}.MaxOrder= MaxOrder-1;
        THERING{HCIndex(i)}.PolynomA(MaxOrder) = 0; % avoid memory leaks
        THERING{HCIndex(i)}.PolynomB(MaxOrder) = 0;
        for j = 1:length(HCerr),
            k     = HCerr{j}.order;
            xi    = HCerr{j}.xi;
            dBoBn = HCerr{j}.dBoBn;
            THERING{HCIndex(i)}.PolynomB(k) = dBoBn*(hcorr_strength(i)+THERING{ATi.COR(i)}.KickAngle(1))*xi;
        end
    end    
end
...
if vcorFlag
    ...
    fid   = fopen('/Users/nadolski/codes/tracy/maille/soleil/corv57.txt','r');
    vcorr = fscanf(fid,'%f');
    ...
    for i = 1:length(VCIndex),
        THERING{VCIndex(i)}.MaxOrder= MaxOrder-1;
        THERING{VCIndex(i)}.PolynomA(MaxOrder) = 0; % avoid memory leaks
        THERING{VCIndex(i)}.PolynomB(MaxOrder) = 0;
        for j = 1:length(VCerr),
            k     = VCerr{j}.order;
            xi    = VCerr{j}.xi;
            dBoBn = VCerr{j}.dBoBn;
            THERING{VCIndex(i)}.PolynomB(k) = dBoBn*vcorr_strength(i)*xi;
        end        
    end
end
