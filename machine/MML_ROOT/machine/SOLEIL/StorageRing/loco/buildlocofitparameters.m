function [LocoMeasData, BPMData, CMData, RINGData, FitParameters, LocoFlags] = buildlocofitparameters(FileName, THERING)
...
ModeCell = {'Fit by family (10 families)', 'Fit magnets by power supply (160)', ...
    'Fit selected quad', 'Fit magnets by power supply (163) Nanoscopium', ...
    'Impedance QSD', '163 + Impedance QSD', 'WSV50 10 quad Q6 to Q10', 'WSV50 8 quad Q6 to Q9 ', ...
    'WSV50 10 quad Q6 to Q8', '163 quad + W164', 'W164'};
[ButtonName, OKFlag] = listdlg('Name','BUILDLOCOINPUT','PromptString', ...
    {'Fit Parameter Selection:','(Not including skew quadrupoles)'}, ...
    'SelectionMode','single', 'ListString', ModeCell, 'ListSize', [350 125], 'InitialValue', 4);
if OKFlag ~= 1
    ButtonName = 1;
end
drawnow;
...
switch ButtonName
    ...
    case 4  % Fit by power supply with Nanoscopium: 163 quads
        n = 0;
        FitParameters.Deltas = [];
        FitParameters.Values = [];
    ...
    case 5  % Fit by power supply in the middle of straight sections
        n = 0;
        FitParameters.Deltas = [];
        FitParameters.Values = [];
        ...
        % K-values
        for k=1:1,
            QI = findcells(THERING,'FamName','QSD');
            for loop=1:length(QI)
                n = n + 1;
                FitParameters.Params{n} = mkparamgroup(THERING, QI(loop), 'K');
            end
            FitParameters.Values = [FitParameters.Values; getcellstruct(THERING, 'K', QI)];
            FitParameters.Deltas = [FitParameters.Deltas; 1e2 * ones(length(QI),1)];
        end
    case 6  %% Fit by power supply in the middle of straight sections
        n = 0;
        FitParameters.Deltas = [];
        FitParameters.Values = [];
    ...
    case 7  % Symmetrization for Wiggler WSV50 in SDC03 with 5+5quadrupoles
        n = 0;
        FitParameters.Deltas = [];
        FitParameters.Values = [];

        QI = family2atindex('Q6', [3 1; 3 2]);
        for loop=1:length(QI)
            n = n + 1;
            FitParameters.Params{n} = mkparamgroup(THERING, QI(loop), 'K');
        end
   ...
   case 8  % Symmetrization for Wiggler WSV50 in SDC03 with 5+5quadrupoles
        n = 0;
        FitParameters.Deltas = [];
        FitParameters.Values = [];
 ...
 case 9  % Symmetrization for Wiggler WSV50 in SDC03 with 5+5quadrupoles
        n = 0;
        FitParameters.Deltas = [];
        FitParameters.Values = [];
        ...
        FitParameters.Values = [FitParameters.Values; getcellstruct(THERING, 'K', QI)];
            FitParameters.Deltas = [FitParameters.Deltas; NaN * ones(length(QI),1)];
 case 10  % Symmetrization for Wiggler WSV164 + 163 quad
        n = 0;
        FitParameters.Deltas = [];
        FitParameters.Values = [];
        ...
        QI = findcells(THERING,'FamName','QW164');
        for loop=1:length(QI)
            n = n + 1;
            FitParameters.Params{n} = mkparamgroup(THERING, QI(loop), 'K');
        end
    ...
    case 11  % Symmetrization for Wiggler WSV164
        n = 0;
        FitParameters.Deltas = [];
        FitParameters.Values = [];
...
ModeCell = {'Fit at Sextupoles', 'Fit By Power Supply', 'HU36 SDM15', ...
...
switch ButtonName
    ...
    case 3 %'HU36 SDC15'
        ...
        QTI = family2atindex('QT',[15 3 ; 15 4 ; 15 5 ; 15 6]);
        for loop=1:length(QTI),
            n = n + 1;
            FitParameters.Params{n} = mkparamgroup(THERING, QTI(loop), 'KS1');
        end
