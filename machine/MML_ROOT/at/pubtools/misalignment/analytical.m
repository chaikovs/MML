...
idip = findcells(THERING,'FamName','dip');
betax_dip = mach.betax(idip);
betay_dip = mach.betay(idip);
dip_amp_x = THERING{idip(1)}.BendingAngle*sqrt(mach.betax)/(2^(3/2)*sin(pi*mach.nux(end)))*sqrt(sum(betax_dip)); 
dip_amp_y = THERING{idip(1)}.BendingAngle*sqrt(mach.betay)/(2^(3/2)*sin(pi*mach.nuy(end)))*sqrt(sum(betay_dip)); 
