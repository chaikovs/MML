...
/* Modification: Laurent S. Nadolski, April 7th, 2007
 * Add check on argument otherwise it crashes with no argument
 * April 22nd, 2011. Removed Limit for coordinates ctau
 */
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if(NewLatticeFlag || FirstCallFlag)
    {
        ...
        /* Pointer to integer maps of Element data filelds used by the tracking function */
     ...
     * push particles and build persistent data structures (field_numbers_ptr)
     * along the way
     */
    ...
    {   for(n=0;n<refpts[num_refpts-1]-1;n++)
        {
            if(methods_table[n])
                ...
                /* if(!mxIsNaN(DblBuffer[0]))
                    mexPrintf("Turn # %d, Element #%d\tPassMethod: '%s'\n", m+2,  n, method_names[n]); */ /* LSN */            
                }
            else
            {
                ptr2mxptr[0] = mxPtrELEM[n];
        ...
        {   for(n=0;n<num_elements;n++)
            {
                ...
                if(methods_table[n])
                {
                    dummy = (*methods_table[n])(mxPtrELEM[n],field_numbers_ptr[n],DblBuffer,num_particles,USE_LOCAL_COPY);
                   /* if(!mxIsNaN(DblBuffer[0]))
                        mexPrintf("x= %.8e px= %.8e y= %.8e py= %.8e delta= %.8e ctau= %.8e\n", DblBuffer[0], DblBuffer[1], DblBuffer[2], DblBuffer[3], DblBuffer[4], DblBuffer[5]); *//* LSN */
                }
                else
                {
                    ptr2mxptr[0] = mxPtrELEM[n];
            ...
            for(p=0;p<np6;p+=6) /* check limits */
                if(!mxIsNaN(DblBuffer[p]))
                    for(n=0;n<5;n++) /* instead of 6 coordinates */
                        if(!mxIsFinite(DblBuffer[p+n]) || DblBuffer[p+n]>LIMIT_AMPLITUDE || DblBuffer[p+n]<-LIMIT_AMPLITUDE)
                        {
                markaslost(DblBuffer+p); /* LSN */
                 mexPrintf("ATPASS: Particle Lost\n");
                break;
                        }
