function [SP, tout, DataTime, ErrorFlag] = getsptl(Family, varargin)
...
%  [AM, tout, DataTime] = getsp(DataStructure, t, FreshDataFlag, TimeOutPeriod)
%
...
if iscell(Family)
    [SP, tout, DataTime, ErrorFlag] = getpvtl(Family, 'Setpoint', varargin{:});
else
    ...
    if nargout < 3
        if FamilyIndex
            % Family name method
            [SP, tout] = getpvtl(Family, 'Setpoint', varargin{:});
        else
            % ChannelName method
            [SP, tout] = getpvtl(Family, '', varargin{:});
        end
    else
        if FamilyIndex
            % Family name method
            ...
            % ChannelName method
            [SP, tout, DataTime, ErrorFlag] = getpvtl(Family, '', varargin{:});
        end
    end
end
