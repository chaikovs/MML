function ErrorFlag = stepsptl(Family, varargin)
...
if iscell(Family)
    ErrorFlag = setpvtl('Inc', Family, 'Setpoint', varargin{:});
else
    [FamilyIndex, AO] = isfamily(Family);

    if FamilyIndex
        % Family name method
        ErrorFlag = setpvtl('Inc', AO, 'Setpoint', varargin{:});
    else
        % ChannelName or TangoName method
        ErrorFlag = setpvtl('Inc', Family, '', varargin{:});
    end
end
