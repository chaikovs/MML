function [AM, tout, DataTime, ErrorFlag] = getpvtest(varargin)
...
%     For example, [d, tout] = getpv('BPMx', 'Monitor', [], 0:eps:100*eps);
            ...
            for j = 1:size(varargin{1},2)
                ...
                if StructOutputFlag
                    if itime == 1
                        ...
                        [AM{i,j}, tout(1,itime), DataTime{i,j}, ErrorFlagNew] = getpvtest(Inputs1{:}, 'Struct', InputFlags{:});
                    else
                        % Add to the Data and time fields
                        [AM{i,j}.Data(:,itime), tmp, AM{i,j}.DataTime(:,itime), ErrorFlagNew] = getpvtest(Inputs1{:}, 'Numeric', InputFlags{:});
                        AM{i,j}.t(1,itime) = t(itime);
                        AM{i,j}.tout(1,itime) = etime(clock, t0);
                        DataTime{i,j}(:,itime) = AM{i,j}.DataTime(:,itime);
                    end
                else
                    [AM{i,j}(:,itime), tmp, DataTime{i,j}(:,itime), ErrorFlagNew] = getpvtest(Inputs1{:}, 'Numeric', InputFlags{:});
                end
...
if FamilyIndex 
    ...
    ExtraTimeDelay = etime(clock, t0);
    if StructOutputFlag || nargout >= 3
        [AM, tout, DataTime, ErrorFlag, DeviceIndex] = local_getpvtest(AO, Field, DeviceList, OutputDataType, t-ExtraTimeDelay, FreshDataFlag, TimeOutPeriod, InputFlags{:});

    
    else
        [AM, tout] = local_getpvtest(AO, Field, DeviceList, OutputDataType, t-ExtraTimeDelay, FreshDataFlag, TimeOutPeriod, InputFlags{:});
    end
    ...
    if ~isempty(FamilyTest)
        ...
        % The DeviceList does not matter (and should not be used)
        ...
        if ~isempty(UnitsFlag)
            error('You cannot change the units when using tango names.')
        end
        if ~(isempty(ModeFlag) || strcmpi(ModeFlag, 'Online'))
            error('Tango names only have an ''Online'' Mode.')
        end
        ...
        if strcmpi(OutputDataType,'String')
            ...
            if StructOutputFlag || nargout >= 3
                [AM, tout, DataTime, ErrorFlag] = local_getbyname(Family, OutputDataType, 0, t-ExtraTimeDelay, FreshDataFlag, TimeOutPeriod, InputFlags{:});
            else
                [AM, tout] = local_getbyname(Family, OutputDataType, 0, t-ExtraTimeDelay, FreshDataFlag, TimeOutPeriod, InputFlags{:});
            end
        end
        tout = tout + ExtraTimeDelay;
...
if ErrorFlag
    ...
    if size(FamilyTest, 1) == 1
        ...
        [AM, tout, DataTime, ErrorFlag] = getpvtest(FamilyTest, Field, DeviceList, t, FreshDataFlag, TimeOutPeriod, 'Numeric', varargin{:});
    else
        ...
        for itime = 1:length(t)
            ...
            for i = 1:size(Family,1)
                [AM(i,itime), tmp, DataTime(i,itime), ErrorFlag1] = getpvtest(Family(i,:), Field, DeviceList(i,:), 'Numeric', varargin{:});
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [AM, tout, DataTime, ErrorFlag] = local_getbyname(ChannelName, OutputDataType, N, t, FreshDataFlag, TimeOutPeriod, varargin)
...
if nargout < 3
    [AM, tout] = getpvonline(ChannelName, OutputDataType, N, t, varargin{:});
else
    [AM, tout, DataTime, ErrorFlag] = getpvonline(ChannelName, OutputDataType, N, t, varargin{:});
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [AM, tout, DataTime, ErrorFlag, DeviceIndex] = local_getpvtest(AO, Field, DeviceList, OutputDataType, t, FreshDataFlag, TimeOutPeriod, varargin)
%  INPUTS
...
if ~isfield(AO, Field);
    ...
    if strcmpi(Mode,'Simulator') || strcmpi(Mode,'Model')
        ExtraTimeDelay = etime(clock, t0);
        [AM, tout, DataTime, ErrorFlag] = getpvmodeltest(AO.FamilyName, Field, DeviceList, t-ExtraTimeDelay, 'Physics', 'Numeric', InputFlags{:});
        tout = tout + ExtraTimeDelay;
    ...
    ExtraTimeDelay = etime(clock, t0);
    if nargout < 3
        [AM, tout] = getpvonline(Tango, OutputDataType, N, t-ExtraTimeDelay, FreshDataFlag, TimeOutPeriod, InputFlags{:});
    else
        [AM, tout, DataTime, ErrorFlag] = getpvonline(Tango, OutputDataType, N, t-ExtraTimeDelay, FreshDataFlag, TimeOutPeriod, InputFlags{:});
    end
    tout = tout + ExtraTimeDelay;
    return
end
...
elseif strcmpi(Mode,'online')
    ...
    if strcmpi(AOField.DataType,'Scalar')
        ...
        ExtraTimeDelay = etime(clock, t0);
        if nargout < 3
            [AM, tout] = getpvonline(AOField.TangoNames(DeviceIndex,:), Field, OutputDataType, N, t-ExtraTimeDelay, InputFlags{:});
        else
            [AM, tout, DataTime, ErrorFlag] = getpvonline(AOField.TangoNames(DeviceIndex,:), Field, OutputDataType, N, t-ExtraTimeDelay, InputFlags{:});
        end
        tout = tout + ExtraTimeDelay;
    ...
    elseif strcmpi(AOField.DataType, 'Vector')
        ...
        for itime = 1:length(t)
            ...
            % Get data
            if nargout >= 3
                [tmp, tout, DataTime(:,itime), ErrorFlag] = getpvonline(TangoNames, 'Vector', N, InputFlags{:});
            else
        ...
        if FreshDataFlag & length(t) == 1
            ...
            while FreshDataCounter
                if nargout >= 3
                    [AM, tout, DataTime, ErrorFlag] = getpvonline(TangoNames, 'Vector', N, InputFlags{:});
                else
    ...
    elseif strcmpi(AOField.DataType,'Matrix')
        ...
        for iDev = 1:length(DeviceIndex)
            ...
            [AM(iDev,:), tout, DataTime(iDev,:), ErrorFlag(iDev,1)] = getpvonline(AOField.TangoNames(DeviceIndex(iDev),:), 'Matrix', N, InputFlags{:});
        end
...
elseif strcmpi(Mode,'Special')
    ...
    elseif isfield(AOField, 'SpecialFunction')
        ...
        error(sprintf('No special function specified for Family %s (getpvtest).', AO.FamilyName));
    end
...
elseif strcmpi(Mode,'Simulator') || strcmpi(Mode,'Model')
    ...
    [AM, tout, DataTime, ErrorFlag] = getpvmodeltest(AO, Field, DeviceList, t-ExtraTimeDelay, AO.(Field).Units, 'Numeric');
    tout = tout + ExtraTimeDelay;
