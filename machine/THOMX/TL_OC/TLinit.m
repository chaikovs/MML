...
%AD.SubMachine    = 'TL_OC'; % Machine Name modifiée par S.Chancé 12/03/14
...
nb = 5;%modification du nombre de BPMs pour le nouveau lattice de la correction d'orbite 12/03/14 S.Chancé
AO.(ifam).DeviceList =[1 1; 1 2; 1 3; 1 4; 1 5]; 
AO.(ifam).ElementList = (1:nb)';
AO.(ifam).DeviceName(:,:)               = {'TL/DG-A/BPM.010'; 'TL/DG-E/BPM.020'; 'TL/DG-H/BPM.030'; 'TL/DG-I/BPM.040'; 'TL/DG-I/BPM.050'};%modif nombres S.Chancé 12/03/14
...
%varlist.BEND3={
%     1  [ 1  1] 'TL/ME/DP.051-DC' 1 'BEND3.01' [+0 +580]
%};

for k = 1:2, %Passage de 2 à 3 pour inclure le septum dans TL_OC 07/04/14 et de 3 à 2 le 08/04/14 pour le septum
...
for ik = 1:4 %Augmentation du nombre de correcteur pour test correction orbite 19/03/14 S.Chancé    
...
nb                             = length(AO.(ifam).DeviceName);
AO.(ifam).Monitor.Range(:,:)     = repmat([-10 10
    ],nb,1);
AO.(ifam).Monitor.TangoNames   = strcat(AO.(ifam).DeviceName, '/current');
AO.(ifam).Status               = ones(nb,1);
AO.(ifam).Monitor.ModelVal     = zeros(1,nb);
...
AO.(ifam).DeviceList             = [1 1; 1 2; 1 3; 1 4];%Augmentation correcteurs de 4 a 6 19/03/14 S.Chancé
AO.(ifam).ElementList            = (1:4)';%Augmentation correcteurs de 4 a 6 19/03/14 S.Chancé
%tango_group_add(AO.(ifam).GroupId, AO.(ifam).DeviceName');
