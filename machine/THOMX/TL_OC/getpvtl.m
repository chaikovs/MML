function [AM, tout, DataTime, ErrorFlag] = getpvtl(varargin)
            ...
            for j = 1:size(varargin{1},2)
                ...
                if StructOutputFlag
                    if itime == 1
                        ...
                        [AM{i,j}, tout(1,itime), DataTime{i,j}, ErrorFlagNew] = getpvtl(Inputs1{:}, 'Struct', InputFlags{:});
                    else
                        % Add to the Data and time fields
                        [AM{i,j}.Data(:,itime), tmp, AM{i,j}.DataTime(:,itime), ErrorFlagNew] = getpvtl(Inputs1{:}, 'Numeric', InputFlags{:});
                        AM{i,j}.t(1,itime) = t(itime);
                        AM{i,j}.tout(1,itime) = etime(clock, t0);
                        DataTime{i,j}(:,itime) = AM{i,j}.DataTime(:,itime);
                    end
                else
                    [AM{i,j}(:,itime), tmp, DataTime{i,j}(:,itime), ErrorFlagNew] = getpvtl(Inputs1{:}, 'Numeric', InputFlags{:});
                end
...
if FamilyIndex 
    ...
    ExtraTimeDelay = etime(clock, t0);
    if StructOutputFlag || nargout >= 3
        [AM, tout, DataTime, ErrorFlag, DeviceIndex] = local_getpvtl(AO, Field, DeviceList, OutputDataType, t-ExtraTimeDelay, FreshDataFlag, TimeOutPeriod, InputFlags{:});

    
    else
        [AM, tout] = local_getpvtl(AO, Field, DeviceList, OutputDataType, t-ExtraTimeDelay, FreshDataFlag, TimeOutPeriod, InputFlags{:});
    end
...
if ErrorFlag
    ...
    if size(FamilyTest, 1) == 1
        ...
        [AM, tout, DataTime, ErrorFlag] = getpvtl(FamilyTest, Field, DeviceList, t, FreshDataFlag, TimeOutPeriod, 'Numeric', varargin{:});
    else
        ...
        for itime = 1:length(t)
            ...
            for i = 1:size(Family,1)
                [AM(i,itime), tmp, DataTime(i,itime), ErrorFlag1] = getpvtl(Family(i,:), Field, DeviceList(i,:), 'Numeric', varargin{:});
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [AM, tout, DataTime, ErrorFlag, DeviceIndex] = local_getpvtl(AO, Field, DeviceList, OutputDataType, t, FreshDataFlag, TimeOutPeriod, varargin)
%  INPUTS
...
if ~isfield(AO, Field);
    ...
    if strcmpi(Mode,'Simulator') || strcmpi(Mode,'Model')
        ExtraTimeDelay = etime(clock, t0);
        [AM, tout, DataTime, ErrorFlag] = getpvmodeltl(AO.FamilyName, Field, DeviceList, t-ExtraTimeDelay, 'Physics', 'Numeric', InputFlags{:});
        tout = tout + ExtraTimeDelay;
...
elseif strcmpi(Mode,'Special')
    ...
    elseif isfield(AOField, 'SpecialFunction')
        ...
        error(sprintf('No special function specified for Family %s (getpvtl).', AO.FamilyName));
    end
...
elseif strcmpi(Mode,'Simulator') || strcmpi(Mode,'Model')
    ...
    [AM, tout, DataTime, ErrorFlag] = getpvmodeltl(AO, Field, DeviceList, t-ExtraTimeDelay, AO.(Field).Units, 'Numeric');
    tout = tout + ExtraTimeDelay;
