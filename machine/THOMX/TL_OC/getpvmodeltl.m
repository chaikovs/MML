function [AM, tout, DataTime, ErrorFlag] = getpvmodeltl(varargin)
...
%
% Need to customized for ThomX in the future...(with the real devices (mangets...) parameters)
%  by Jianfeng Zhang @ LAL,
% 07/10/2013
%
...
if iscell(Family)
    for i = 1:length(Family)
        if length(varargin) < 2
            [AM{i}, tout{i}, DataTime{i}, ErrorFlag{i}] = getpvmodeltl(Family{i}, UnitsFlag{:});
        elseif length(varargin) < 3
            if iscell(Field)
                [AM{i}, tout{i}, DataTime{i}, ErrorFlag{i}] = getpvmodeltl(Family{i}, Field{i}, UnitsFlag{:});
            else
                [AM{i}, tout{i}, DataTime{i}, ErrorFlag{i}] = getpvmodeltl(Family{i}, Field, UnitsFlag{:});
            end
        else
            if iscell(Field)
                if iscell(DeviceList)
                    [AM{i}, tout{i}, DataTime{i}, ErrorFlag{i}] = getpvmodeltl(Family{i}, Field{i}, DeviceList{i}, UnitsFlag{:}, t);
                else
                    [AM{i}, tout{i}, DataTime{i}, ErrorFlag{i}] = getpvmodeltl(Family{i}, Field{i}, DeviceList, UnitsFlag{:}, t);
                end
            else
                if iscell(DeviceList)
                    [AM{i}, tout{i}, DataTime{i}, ErrorFlag{i}] = getpvmodeltl(Family{i}, Field, DeviceList{i}, UnitsFlag{:}, t);
                else
                    [AM{i}, tout{i}, DataTime{i}, ErrorFlag{i}] = getpvmodeltl(Family{i}, Field, DeviceList, UnitsFlag{:}, t);
                end
            end
        end
    end
    ...
    if isfield(AT, 'ATParameterGroup')
        ...
        if any(strcmpi(AT.ATType, {'x','BPMx','Horizontal','y','BPMy', 'z','BPMz','Vertical','ClosedOrbit'}))
            ...
            if isempty(CavityState)
                ...
                Orbit = findsyncorbittl(THERING, 0, ATIndexList);
            else
                if strcmpi(deblank(CavityState(1,:)), 'On') || isradiationon
                    ...
                    Orbit = findsyncorbittl(THERING, -C*DeltaRF*CavityHarmNumber/CavityFrequency^2, ATIndexList);
