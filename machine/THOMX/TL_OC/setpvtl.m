function ErrorFlag = setpvtl(varargin)
...
if iscell(varargin{1})
    ...
    for i = 1:size(varargin{1},1)
        for j = 1:size(varargin{1},2)
            ...
            ErrorFlagNew = setpvtl(Inputs1{:}, InputFlags{:});
...
if isempty(Family)
    ...
    if size(FamilyTest, 1) == 1
        ...
        ErrorFlag = setpvtl(FamilyTest, Field, NewSP, DeviceList, WaitFlag, InputFlags{:});
    else
        % Multiple families (loop)
        % It has to be in a loop for the simulator and to pickup special functions
        for i = 1:size(Family,1)
            ErrorFlag1 = setpvtl(Family(i,:), Field, NewSP(i,:), DeviceList(i,:), 0, InputFlags{:});
            ErrorFlag = ErrorFlag | ErrorFlag1;
        end
        if WaitFlag
            for i = 1:size(Family,1)
                ErrorFlag1 = setpvtl(Family(i,:), Field, NewSP(i,:), DeviceList(i,:), WaitFlag, InputFlags{:});
                ErrorFlag = ErrorFlag | ErrorFlag1;
            end
        end
    end
...
% Incremental change in the setpoint
if IncrementalFlag
    SPold = getpvtl(Family, Field, DeviceList, AO.(Field).Units, AO.(Field).Mode, InputFlags2{:});
    if ~any(isnan(SPold))
        NewSP = NewSP + SPold;
    else
        ErrorFlag = -1;
...
ErrorFlag = local_setpvtl(AO, Field, NewSP, DeviceList, WaitFlag, InputFlags2{:});
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function ErrorFlag = local_setpvtl(AO, Field, NewSP, DeviceList, WaitFlag, varargin)
%  INPUTS
...
if ~isfield(AO, Field)
    ...
    if strcmpi(Mode,'Simulator') || strcmpi(Mode,'Model')
        ErrorFlag = setpvmodeltl(AO.FamilyName, Field, NewSP, DeviceList, 'Physics');
        return
    end
                        ...
                        if x(i)
                            ...
                            if ~strcmp(TangoName, TangoNameNext)
                                if isfield(AO,'Monitor') & isfield(AO,'Setpoint')
                                    ...
                                    AM = getamtl(AO.FamilyName,  DeviceList(i,:));
...
elseif strcmpi(Mode,'Special')
    ...
    if isfield(AO.(Field), 'SpecialFunctionSet')
        ...
        error(sprintf('No special function specified for Family %s (setpvtl).', AO.FamilyName));
    end
...
elseif strcmpi(Mode,'Simulator')
    ...
    ErrorFlag = setpvmodeltl(AO, Field, NewSP, DeviceList, AO.(Field).Units);
