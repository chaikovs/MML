...
elseif ModeNumber == 2
    % User mode - High Tune, Top Off injection
    AD.OperationalMode = 'TLlattice_30periods_PlanF_avseptum_rematch';
    AD.Energy = 0.05; % Make sure this is the same as bend2gev at the production lattice!
    ModeName = 'TLlattice_30periods_PlanF_avseptum_rematch';
    OpsFileExtension = '_TLlattice_30periods_PlanF_avseptum_rematch';

    % AT lattice
    AD.ATModel = 'TLlattice_30periods_PlanF_avseptum_rematch';
    eval(AD.ATModel);  %run model for compilersolamor2linb;

    switch2hw;
    elseif ModeNumber == 3
    % User mode - High Tune, Top Off injection
    AD.OperationalMode = 'TLlattice_30periods_PlanF_avseptum_leq_rematch';
    AD.Energy = 0.05; % Make sure this is the same as bend2gev at the production lattice!
    ModeName = 'TLlattice_30periods_PlanF_avseptum_leq_rematch';
    OpsFileExtension = '_TLlattice_30periods_PlanF_avseptum_leq_rematch';

    % AT lattice
    AD.ATModel = 'TLlattice_30periods_PlanF_avseptum_leq_rematch';
    eval(AD.ATModel);  %run model for compilersolamor2linb;
...
AD.Directory.Lattice        = fullfile(MMLROOT, 'machine', 'THOMX', 'TL', 'Lattices', filesep);
