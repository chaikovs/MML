...
function varargout = emit_v2MML(varargin)
%EMIT_V2MML M-file for emit_v2MML.fig
%      EMIT_V2MML, by itself, creates a new EMIT_V2MML or raises the existing
%      singleton*.
%
%      H = EMIT_V2MML returns the handle to a new EMIT_V2MML or the handle to
%      the existing singleton*.
%
%      EMIT_V2MML('Property','Value',...) creates a new EMIT_V2MML using the
%      given property value pairs. Unrecognized properties are passed via
%      varargin to emit_v2MML_OpeningFcn.  This calling syntax produces a
%      warning when there is an existing singleton*.
%
%      EMIT_V2MML('CALLBACK') and EMIT_V2MML('CALLBACK',hObject,...) call the
%      local function named CALLBACK in EMIT_V2MML.M with the given input
%      arguments.
%
...
% Edit the above text to modify the response to help emit_v2MML

% Last Modified by GUIDE v2.5 11-Dec-2014 19:08:53
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @emit_v2MML_OpeningFcn, ...
                   'gui_OutputFcn',  @emit_v2MML_OutputFcn, ...
                   'gui_LayoutFcn',  [], ...
                   'gui_Callback',   []);
...
function emit_v2MML_OpeningFcn(hObject, eventdata, handles, varargin)

% Choose default command line output for emit_v2MML
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes emit_v2MML wait for user response (see UIRESUME)
% uiwait(handles.figure1);


function varargout = emit_v2MML_OutputFcn(hObject, eventdata, handles)
...
dimensions=dim_theo_MML(Q_min,Q_max,n_pas, emit_x, emit_y);
...
%% matrice de i mesures T11^2 2T11T12 T12^2 %%%%% a inclure directement dans le scan des dimension si camera.
    for i=1:length(paramk)
        setpv('QP1L',paramk(i));
        TD1=twissline(THERING,0,THERING{1}.TwissData,1:17); %%%%% Attention mettre end quand maille bien défiie avec bend intégré
        Mat=cat(1,TD1.M44);
        Mtot=Mat(end-3:end,:);
...
function checkbox_MML_Callback(hObject, eventdata, handles)


function checkbox_Image_Callback(hObject, eventdata, handles)


function checkbox_Tango_Callback(hObject, eventdata, handles)
