...
quadscales = FitParameters(1).Values(1:end-13)./FitParameters(end).Values(1:end-13);
figure
plot(quadscales)
...
SkewK_LOCO = FitParameters(end).Values(end-12:end);
...
figure
plot(current)
title('Skew quadrupole current changes')
...
%     from the list.
stepsp('SkewQuad',current,SQList)
