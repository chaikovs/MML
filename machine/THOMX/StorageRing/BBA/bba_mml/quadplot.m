...
%
%  See also quadcenter, quaderrors
...
% TO SEE
% At SOLEIL noise is very small, constraint too large
%OutlierFactor = QMS.OutlierFactor;     % if abs(data - fit) > OutlierFactor*BPMsdt, then remove that BPM
OutlierFactor =  1e-3;     % if abs(data - fit) > OutlierFactor, then remove that BPM
...
if all(FigureHandle ~= 0) 
    ...
    hold on;
    % just data with corrector value at Inominal - deltaI
...
%        plot(xx, y); hold on  
    ...
    if max(abs(y - merit(i,:)')) > OutlierFactor;    % OutlierFactor was absolute max(abs(y - merit(i,:)')) > OutlierFactor
    %if max(abs(y - merit(i,:)')) > OutlierFactor * sigmaBPM(i) %Laurent
...
fprintf('   2. Mean = %+.5f %s, STD = %.5f %s, %2d points with condition 1 and abs(fit - measured data) > %.4f (BPM outlier)\n', m2, BPMUnitsString, s2, BPMUnitsString, length(iBad), OutlierFactor);
...
if all(FigureHandle ~= 0)
    ...
    addlabel(0, 0, sprintf('Offset %s(%d,%d)=%f, {\\Delta}%s(%d,%d)=%f, {\\Delta}%s(%d,%d)=%f', QMS.BPMFamily, QMS.BPMDev, QMS.Center, QMS.QuadFamily, QMS.QuadDev, QMS.QuadDelta, QMS.CorrFamily, QMS.CorrDevList(1,:), QMS.CorrDelta(1)),10);
end
