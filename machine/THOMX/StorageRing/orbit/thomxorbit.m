...
%
%=============================================================
% Instruction sequence to launch orbit program for ThomX
% Laurent S. Nadolski 
...
%
% See Also orbgui, bpmgui, corgui, respgui, sortbpms, soleilrestore, sortcors, restoreorbit
...
%change directory to THOMX
vpath = getfamilydata('Directory','Orbit');
%vpath = '/Users/ichaikov/Documents/MATLAB/SVN/mml_thomx/MML/machine/THOMX/StorageRing/orbit/';
cd(vpath);
...
BPM = bpminit_orb;%BPMInit_Orb;
COR = corinit_orb;%CORInit_Orb;

%SYS parameters are required for initialization
SYS.machine    = 'THOMX';
if ModeFlag
SYS.restorefile= 'thomxrestore';
else
    SYS.restorefile = 'thomxrestore';
end
...
SYS.lifetime   = 0;         % Lifetime in hours 
...
BPM = reforb2zero(BPM);%RefOrb2Zero(BPM);
...
switch getmode(COR(1).AOFamily);
    case 'Online'
        ...
        set(SYS.orbfig,'Name', ...
            'THOMX Storage Ring Orbit Correction Interface (Online)');
        set(SYS.modecolor,'BackGroundColor','r','String','Online');       
    otherwise % Assume simulator mode
        SYS.mode   = 'Simulator';
        set(SYS.sim, 'Checked', 'On');
        set(SYS.orbfig,'Name', ...
            'THOMX Storage Ring Orbit Correction Interface (Simulator)');
...
disp('   Checking BPMs...');
[BPM(1).status BPM(2).status] = thomxbpmcheck; %get status from TANGO or sim
...
[COR(1).status COR(2).status] = thomxcorcheck;
...
% Corrector in Harware units {default}
