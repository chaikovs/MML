...
BPMxList = [];   
BPMzList = [];   
T = 0.03*60; %total monitoring time
...
if ModelFlag
    ...
    if isfamily('DCCT')
        ...
        y0 = AM{2}.Data;
        for i = 1:length(t)
            AM{1}.Data(:,i) = x0(:,i) + 10e-3*randn(size(AM{1}.DeviceList,1),1); 
            AM{2}.Data(:,i) = y0(:,i) + 10e-3*randn(size(AM{2}.DeviceList,1),1);% <================== BPM STD
        end
        DCCT = AM{3};
    else
