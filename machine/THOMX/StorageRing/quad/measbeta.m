...
%MEASBETA - Measure betatron functions 
%
%  INPUTS
%  1. Quadrupole family name {Default : All}
%  Optional input variable:
%  'Display', 'NoDisplay':  plot the measured beatron function or not
%   'Archive', 'NoArchive': save the betatron function or not 
%
%
...
%  Jianfeng zhang @ LAL, 21/02/2014
%    Fix the bug to display the measured beta.  

DisplayFlag = 1;
ArchiveFlag = 1;
FileName = '';
ModeFlag = '';  % model, online, manual, or '' for default mode
waittime = 1; %seconds taken into account for simulator and Online
OutputFlag = 1;
...
for i = length(varargin):-1:1
    ...
    elseif strcmpi(varargin{i},'Display')
        DisplayFlag = 1;
        varargin(i) = [];
    elseif strcmpi(varargin{i},'NoDisplay')  % bug fixed, by Zhang
                                             % @ LAL, 21/02/2014
         DisplayFlag = 0;
        varargin(i) = [];
...
if ArchiveFlag
    if isempty(FileName)
        ...
        if isempty(DirectoryName)
            DirectoryName = fullfile(getfamilydata('Directory','DataRoot'), 'Response', 'BPM');
        else
